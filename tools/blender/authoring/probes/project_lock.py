"""Process-owned project lock for the mock's single-platform session profile."""
import ctypes
...
class ProjectLock:
    def __init__(self, project, lockfile):
        ...
        if os.name == 'nt':
            ...
            self.handle = self.kernel.CreateMutexW(None, True, 'Local\\LuminumbraAuthorMock_' + identity)
            error = ctypes.get_last_error()
            if not self.handle:
                raise OSError(error, 'Cannot create the MOCK project mutex')
            if error == 183:  # ERROR_ALREADY_EXISTS
                self.kernel.CloseHandle(self.handle)
                raise BlockingIOError('A Windows MOCK process already owns this project')
        else:
            import fcntl
