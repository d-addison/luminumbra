...
hand-duplicate. It START-s with render.moonlight (OPS-08 / FR-B-007) to prove the pipeline
...
# Config params migrated to the GENERATED shared-constant header (OPS-08 / FR-B-007).
...
def emit_constants_header(schema: dict) -> str:
    """Emit the shared-constant header for the CONSTANT_MIGRATIONS subset (FR-B-007).
    ...
    lines.append("//")
    lines.append("// Shared config-mirrored constants (spec 020 FR-B-007 / OPS-08). Each value is the")
    ...
    lines.append("//")
    lines.append("// Shader-side twin (TODO when the shader consumer migrates): emit the SAME defaults")
    lines.append("// into a generated GLSL/Slang include from this same table, e.g.")
    lines.append("//   #define LUMIN_MOONLIGHT_STRENGTH 0.0")
    lines.append("//   #define LUMIN_MOONLIGHT_COLOR vec3(0.6, 0.7, 1.0)")
    lines.append("// keeping the numbers sourced from ConfigSchema.json (never hand-typed in a shader).")
    lines.append("")
