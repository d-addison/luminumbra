...
function Test-WorldLoadBounded {
    ...
    # load path dispatches surface builds in bounded 64-job sub-batches with
    # named per-batch progress, so a wedge surfaces as a named report + a
    # blown per-load bound, never a silent hang. Heavyweight (~minutes in
    # debug) -> a first-class MODE, deliberately NOT in -Mode All.
    $exe = "build/$BuildPreset/bin/world_load_bounded_test.exe"
    if (-not (Test-Path $exe)) {
        throw "world-load-bounded gate not built - missing $exe (cmake --build --preset $BuildPreset --target world_load_bounded_test)"
    }
    & $exe --gtest_filter=WorldLoadBounded.*
    if ($LASTEXITCODE -ne 0) {
        throw "world-load-bounded gate FAILED (exit $LASTEXITCODE) - a load blew the 60 s bound or failed outright; the named watchdog phase in the output localizes the wedged sub-batch"
    }
    Write-Host "world-load-bounded gate passed: 20 cold interactive loads (radius 12/4) each under 60 s, watchdog armed"
}
