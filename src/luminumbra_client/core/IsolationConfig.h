...
// debug-viz + interactive toggle are explicit v2 deferrals.
//
...
struct IsolationConfig {
    ...
    bool renders(std::uint32_t layer_bit) const { return (layer_mask & layer_bit) != 0u; }
};
...
inline std::string lower_trim(std::string_view tok) {
    ...
    std::string out;
    out.reserve(e - b);
    for (std::size_t i = b; i < e; ++i) out.push_back(to_lower_ascii(tok[i]));
    return out;
}
inline std::uint32_t layer_bit_for(const std::string& name) {
    if (name == "terrain") return IsolationLayer::Terrain;
    if (name == "farfield" || name == "far_field" || name == "far") return IsolationLayer::FarField;
    if (name == "water") return IsolationLayer::Water;
    if (name == "foliage" || name == "grass") return IsolationLayer::Foliage;
    if (name == "particles" || name == "particle") return IsolationLayer::Particles;
    if (name == "skinned" || name == "creatures" || name == "creature") return IsolationLayer::Skinned;
    if (name == "skybox" || name == "sky") return IsolationLayer::Skybox;
    if (name == "aerial" || name == "fog") return IsolationLayer::Aerial;
    if (name == "lightning") return IsolationLayer::Lightning;
...
inline BackdropMode ParseBackdropMode(std::string_view s) {
    ...
    if (v == "void" || v == "black") return BackdropMode::Void;
    if (v == "greenscreen" || v == "green" || v == "chroma") return BackdropMode::Greenscreen;
    if (v == "checker" || v == "checkerboard") return BackdropMode::Checker;
    if (v == "transparent" || v == "alpha") return BackdropMode::Transparent;
    return BackdropMode::Scene;
}

inline const char* BackdropModeName(BackdropMode m) {
    switch (m) {
        case BackdropMode::Scene: return "scene";
        case BackdropMode::Void: return "void";
        case BackdropMode::Greenscreen: return "greenscreen";
        case BackdropMode::Checker: return "checker";
        case BackdropMode::Transparent: return "transparent";
    }
    return "scene";
}

inline IsolationConfig ParseIsolationConfig(std::string_view layers_csv, std::string_view backdrop) {
