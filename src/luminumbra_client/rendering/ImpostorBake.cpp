...
struct BakePart { const char* path; glm::vec3 flatColor; };
...
GLuint CompileBakeProgram(std::string& err) {
    ...
    GLuint vs = compile(GL_VERTEX_SHADER, kVert); if (!vs) return 0;
    GLuint fs = compile(GL_FRAGMENT_SHADER, kFrag); if (!fs) { glDeleteShader(vs); return 0; }
...
// Shared render core: loads the tree parts, renders all octahedral tiles into two KEPT GL_TEXTURE_2D
// (albedo + normal), and returns them + the local bounding sphere. The caller owns the textures.
struct RenderedAtlas { bool ok=false; GLuint albedo=0, normal=0; int size=0; float sphereY=0, radius=0; std::string err; };
RenderedAtlas RenderAtlas(const std::string& rootDir, const RenderPipeline& rp, const OctaImpostorGrid& grid) {
    RenderedAtlas out;
    ...
    glm::vec3 unionC(0.0f); float unionR = 0.0f; bool first = true;
    for (const BakePart& p : kParts) {
        ...
        if (!m || m->indexCount == 0) { out.err = std::string("failed to load tree part: ") + full; return out; }
        ...
        else { const glm::vec3 mid = (unionC + c) * 0.5f; unionR = glm::max(glm::length(mid-unionC)+unionR, glm::length(mid-c)+r); unionC = mid; }
        const RenderPipeline::StaticModelTex* tex = rp.static_model_tex(p.path);
        LoadedPart lp; lp.mesh = std::move(m); lp.layer = tex ? tex->albedoLayer : -1;
        lp.alphaTest = (tex && tex->alphaTest) ? 1 : 0; lp.flat = p.flatColor;
        parts.push_back(std::move(lp));
    }
    if (unionR <= 0.0f) unionR = 1.0f;
    out.sphereY = unionC.y; out.radius = unionR;

    std::string err;
    const GLuint prog = CompileBakeProgram(err);
    if (!prog) { out.err = "bake shader: " + err; return out; }
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        ...
        glDeleteFramebuffers(1, &fbo); glDeleteTextures(1, &out.albedo); glDeleteTextures(1, &out.normal);
    ...
    for (int j = 0; j < n; ++j) {
        for (int i = 0; i < n; ++i) {
            ...
            const glm::mat4 mvp = glm::ortho(-hr, hr, -hr, hr, 0.01f, D + unionR * 2.0f) * glm::lookAt(eye, unionC, up);
            glViewport(i * tile, j * tile, tile, tile);
            glUniformMatrix4fv(locMVP, 1, GL_FALSE, &mvp[0][0]);
            for (const LoadedPart& lp : parts) {
                glUniform1i(locLayer, lp.layer); glUniform1i(locAlpha, lp.alphaTest);
...
ImpostorBakeResult BakeTreeImpostorAtlas(const std::string& outPpmPath, const std::string& rootDir,
    ...
    if (!ra.ok) { out.error = ra.err; return out; }
    ...
    glBindTexture(GL_TEXTURE_2D, ra.albedo); glGetTexImage(GL_TEXTURE_2D, 0, GL_RGB, GL_UNSIGNED_BYTE, rgb.data());
    glBindTexture(GL_TEXTURE_2D, ra.normal); glGetTexImage(GL_TEXTURE_2D, 0, GL_RGB, GL_UNSIGNED_BYTE, nrm.data());
    ...
    double covSum = 0.0; out.min_coverage = 1.0f;
    for (int tj = 0; tj < n; ++tj) for (int ti = 0; ti < n; ++ti) {
        ...
        for (int py = 0; py < tile; ++py) for (int px = 0; px < tile; ++px) {
            ...
            if (!(rgb[idx] == bgR && rgb[idx+1] == 0 && rgb[idx+2] == bgB)) ++covered;
        }
        const float cov = static_cast<float>(covered) / static_cast<float>(tile * tile);
        tileCov[static_cast<std::size_t>(tj) * n + ti] = cov; covSum += cov;
    ...
    { const auto dot = normalPath.find_last_of('.'); if (dot != std::string::npos) normalPath.insert(dot, "_normal"); else normalPath += "_normal"; }
    WritePpm(normalPath, atlas, atlas, nrm);
    out.ok = WritePpm(outPpmPath, atlas, atlas, rgb);
    if (!out.ok) out.error = "failed to write atlas PPM: " + outPpmPath;
    else {
...
ImpostorAtlasTextures BakeTreeImpostorAtlasToTextures(const std::string& rootDir, const RenderPipeline& rp,
