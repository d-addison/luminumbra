...
// Spec 016 (FR-B-001/002/003): typed render-resource handles.
//
...
// (016 OQ-1 lean), NOT a frame graph with automatic barrier insertion / transient
// aliasing. That can be added later if the Vulkan port (spec 014) needs it.
...
struct FboHandle {
    ...
    bool operator==(const FboHandle& o) const { return id == o.id && valid == o.valid; }
};
...
inline FboHandle     default_framebuffer()    { return FboHandle{0u, true}; }
inline BufferHandle  adopt_buffer(u32 gl_id)  { return BufferHandle{gl_id}; }
