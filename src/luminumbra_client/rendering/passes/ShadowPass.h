...
public:
    ...
    const std::unique_ptr<Shader>& tint_shader() const { return m_tint_shader; }
    ...
    unsigned int tint_texture_array() const { return m_tint_texture_array; }

private:
    ShadowMap m_shadow_map;
    std::unique_ptr<Shader> m_shadow_shader;
    // Spec 015 C-1: the tint cascade + its per-cascade FBO (color = tint layer,
