...
// Spec 015 Pillar A (FR-A-001): SUN IRRADIANCE FROM TRANSMITTANCE.
//
...
// physical term makes the sun both DIM and REDDEN across the golden hour (AC-A-002) — no
...
// This is the answer to spec 015 OQ-2 (TOA solar constant + day exposure held at 1.12).
//
...
// the FR-A-001 contract gate exercises the SAME code the frame uses.
inline glm::vec3 SunIrradiance(const glm::vec3& t_now, const glm::vec3& t_ref, float sun_up_factor) {
...
inline glm::vec3 SunBaseHue() { return glm::vec3(1.0f, 0.95f, 0.85f); }
