#pragma once
// In-process FLIP-style perceptual image comparison (GPU-09; spec 016 FR-C.3 /
// FR-D.2). A GL-free, header-only C++ port of tools/flip_diff.py's `backend_luma`
...
//   - GPU-09 (rank 60): the DualBackendFlipInProcessGpu ctest calibrates the
...
//     gives rank 66 an empirical basis for its per-pass threshold.
//   - GPU-03 + GPU-P05 (rank 66): the RHI pilot's RhiPilotParityGpu reuses this
//     exact function to FLIP GL-via-Diligent / native-Vulkan framebuffers
//     against raw-GL goldens, with the thresholds recorded by GPU-09.
//
...
inline const char* BackendName() { return "luma"; }
...
inline FlipResult ComputeLumaFlip(const std::uint8_t* a, const std::uint8_t* b,
