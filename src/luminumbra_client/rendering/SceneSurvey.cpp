...
SurveyTarget frame_target(const std::string& label, glm::vec3 poi, float back, float up, float tod) {
    SurveyTarget t;
    t.label = label; t.poi = poi; t.back = back; t.up = up; t.tod = tod;
    return t;
}
...
void RunSceneSurvey(GLFWwindow* window,
    ...
    int water_cells = 0;
    float best_rock = -1.0f;  glm::vec3 best_rock_pos = spawn; // score = slope * elevation
    float best_grass_h = 0.0f; glm::vec3 best_grass_pos = spawn; bool grass_found = false; int best_grass_biome = -1;
    float best_lake_depth = 0.0f; glm::vec3 best_lake_pos(0.0f); bool lake_found = false;
    ...
    for (int dz = -kHalf; dz <= kHalf; ++dz) {
        for (int dx = -kHalf; dx <= kHalf; ++dx) {
            ...
            elev_min = std::min(elev_min, h); elev_max = std::max(elev_max, h);
            const float wl = w->WaterLevelAt(x, z);
            if (wl > h + 0.3f) {
                ++water_cells;
                const float depth = wl - h;
                if (depth > best_lake_depth) { best_lake_depth = depth; best_lake_pos = glm::vec3(x, wl, z); lake_found = true; }
            ...
            if (rock_score > best_rock) { best_rock = rock_score; best_rock_pos = glm::vec3(x, h, z); }
            // GRASS: flattish, above water, in a VEGETATED biome (highest veg density wins).
            if (h > 1.0f && slope < 0.18f) {
                float veg = 0.0f; int bid = -1;
                if (has_biomes) {
                    bid = static_cast<int>(w->BiomeIdAt(x, z));
                    if (bid >= 0 && bid < 256) { ++biome_hist[static_cast<std::size_t>(bid)]; }
                ...
                if (score > best_grass_h) { best_grass_h = score; best_grass_pos = glm::vec3(x, h, z); grass_found = true; best_grass_biome = bid; }
            }
        }
    }
    {
        std::string top_biomes;
        for (int i = 0; i < 256; ++i) if (biome_hist[i] > 0) top_biomes += " " + std::to_string(i) + ":" + std::to_string(biome_hist[i]);
        LUMINUMBRA_CORE_INFO("SceneSurvey world scan: elev [{:.0f}..{:.0f}], water_cells={}, biomes_enabled={}, biome_hist={}",
                             elev_min, elev_max, water_cells, has_biomes, top_biomes);
    }
    ...
    // --- WATERFALLS (framed from downstream, looking UP at the crest, daytime) -
    {
        ...
        std::sort(sites.begin(), sites.end(),
                  [](const WaterfallSite& a, const WaterfallSite& b) { return a.drop_height > b.drop_height; });
        const int n = std::min<int>(2, static_cast<int>(sites.size()));
        for (int i = 0; i < n; ++i) {
            const glm::vec3 look = (sites[i].crest + sites[i].foot) * 0.5f;
            targets.push_back(frame_target("waterfall_" + std::to_string(i), look,
                                           16.0f + sites[i].drop_height * 1.1f,
                                           sites[i].drop_height * 0.45f + 4.0f, kDayTod));
            LUMINUMBRA_CORE_INFO("SceneSurvey waterfall {}: drop {:.1f}m at ({:.0f},{:.0f}) lake_outlet={}",
                                 i, sites[i].drop_height, sites[i].crest.x, sites[i].crest.z, sites[i].lake_outlet);
        }
        if (n == 0) LUMINUMBRA_CORE_INFO("SceneSurvey: NO waterfall sites detected in this world");
    }
    ...
    LUMINUMBRA_CORE_INFO("SceneSurvey rock: score {:.1f} at ({:.0f},{:.0f}) h={:.0f}",
                         best_rock, best_rock_pos.x, best_rock_pos.z, best_rock_pos.y);

    // --- GRASS FIELD (most-vegetated flat cell; eye-level, look across) -------
    {
        SurveyTarget t;
        t.label = "grass_field"; t.poi = best_grass_pos; t.up = 2.2f; t.tod = kDayTod; t.eye_level = true;
        targets.push_back(t);
        LUMINUMBRA_CORE_INFO("SceneSurvey grass: biome {} at ({:.0f},{:.0f}) found={}",
                             best_grass_biome, best_grass_pos.x, best_grass_pos.z, grass_found);
    }
    ...
    if (lake_found) {
        ...
        LUMINUMBRA_CORE_INFO("SceneSurvey lake: depth {:.1f}m at ({:.0f},{:.0f})",
                             best_lake_depth, best_lake_pos.x, best_lake_pos.z);
    } else {
    ...
    for (const SurveyTarget& t : targets) {
        ...
        // Settle: render frames so foliage/water/atmosphere + mesh uploads catch up. Longer for water
        // (a river needs ticks to source-fill + flow + the surface mesh to regenerate).
        for (int i = 0; i < 130; ++i) {
            w->update(session.GetRegistry(), anchor, phys);
            pipeline.render_frame(session.GetRegistry(), *w, camera, 1.0f / 60.0f, wireframe);
        }
        // Spec 009 probe: report the live sim's max water depth (mm) so river-fill is verifiable by DATA
        // even when the dark survey scene / pixel heuristic can't show it.
        LUMINUMBRA_CORE_INFO("SceneSurvey '{}': live max water depth = {} mm", t.label,
        ...
        std::vector<unsigned char> px(static_cast<std::size_t>(fbw) * static_cast<std::size_t>(fbh) * 3u);
        glReadBuffer(GL_BACK);
        glPixelStorei(GL_PACK_ALIGNMENT, 1);
        glReadPixels(0, 0, fbw, fbh, GL_RGB, GL_UNSIGNED_BYTE, px.data());
        Luminumbra::Client::ScenarioHarness::WritePixelBufferPpm(out_dir / (t.label + ".ppm"), fbw, fbh, px);

        LUMINUMBRA_CORE_INFO("SceneSurvey captured '{}' cam=({:.0f},{:.0f},{:.0f}) yaw={:.0f} pitch={:.0f}",
                             t.label, cam.x, cam.y, cam.z, camera.Yaw, camera.Pitch);
        index.push_back({{"label", t.label},
                         {"cam", {cam.x, cam.y, cam.z}},
                         {"poi", {t.poi.x, t.poi.y, t.poi.z}},
                         {"yaw", camera.Yaw}, {"pitch", camera.Pitch}, {"tod", t.tod},
                         {"ppm", t.label + ".ppm"}, {"scan", t.label + ".json"}});
    }

    std::ofstream(out_dir / "survey.json") << index.dump(2) << "\n";
    LUMINUMBRA_CORE_INFO("SceneSurvey: wrote {} POI captures -> {}", targets.size(), out_dir.string());
}
