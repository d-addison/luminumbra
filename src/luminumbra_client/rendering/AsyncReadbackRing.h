...
// Spec 017-A (FR-A-001/002/006): the async GPU->CPU readback ring.
//
// THE PROBLEM (spec 017 F4): render code reads GPU buffers back with in-frame
// SYNCHRONOUS calls -- glClientWaitSync(..., GL_TIMEOUT_IGNORED) + glMapBuffer(
// GL_READ_ONLY) (the GPU-SDF path) and glGetBufferSubData (the foliage gate
...
//
// BACKEND-AGNOSTIC INTERFACE (FR-A-006): the public API is submit/poll/consume
// in plain types -- no GL names in any signature a caller sees (the source
// buffer is an opaque `unsigned int` handle today; spec 014's RHI backs it with
...
// instance_hash now; exposure metering / GPU-SDF later) -- never world_hash. A
// stale (N-frame-old) result is the contract, not a bug (NFR-004).
//
...
public:
    ...
    // `src_buffer` is the GL buffer name today (an RHI buffer handle behind 014).
    void copy_region(unsigned int src_buffer, std::ptrdiff_t src_offset,
                     std::ptrdiff_t dst_offset, std::size_t num_bytes);

    // Fence the current slot's copies and advance the cursor. Returns immediately
    // (FR-A-001) -- never waits on the GPU.
    void submit();

    // Non-blocking, NON-DESTRUCTIVE readiness check (FR-A-002): polls every
    ...
    // Non-blocking poll (FR-A-002). Polls every in-flight fence with a ZERO
    ...
    int depth() const { return m_num_slots; }
    std::size_t slot_bytes() const { return m_slot_bytes; }
