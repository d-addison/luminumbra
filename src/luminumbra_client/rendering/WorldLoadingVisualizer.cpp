...
void WorldLoadingVisualizer::Startup(const std::filesystem::path& root_path,
                                     int screen_width,
                                     int screen_height) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        glVertexAttribPointer(MATRIX_ATTRIB_LOCATION + i,
                              4,
                              GL_FLOAT,
                              GL_FALSE,
                              sizeof(ChunkInstanceData),
                              (void*)(offsetof(ChunkInstanceData, transform) + sizeof(Vec4) * i));
        glVertexAttribDivisor(MATRIX_ATTRIB_LOCATION + i, 1);
    }

    const GLuint COLOR_ATTRIB_LOCATION = 5; // 1 + 4 = 5
    glEnableVertexAttribArray(COLOR_ATTRIB_LOCATION);
    glVertexAttribPointer(COLOR_ATTRIB_LOCATION,
                          4,
                          GL_FLOAT,
                          GL_FALSE,
                          sizeof(ChunkInstanceData),
                          (void*)offsetof(ChunkInstanceData, color));
...
void WorldLoadingVisualizer::BeginVisualization(const std::vector<IVec3>& initial_chunks) {
    ...
    for (const auto& coords : initial_chunks) {
        ...
        m_instance_data.push_back(
            {transform, Vec4(0.1f, 0.1f, 0.1f, 0.0f)}); // Start dim and not pulsing
    }

    m_instance_buffer_dirty = true;
    LUMINUMBRA_CORE_INFO("Starting world loading visualization with {} chunks.",
                         initial_chunks.size());
}

void WorldLoadingVisualizer::UpdateChunkState(const IVec3& chunk_coords,
                                              ChunkLoadVisualState state) {
...
void WorldLoadingVisualizer::UpdateAndRender(float delta_time,
    ...
    if (m_camera_angle > 360.0f)
        m_camera_angle -= 360.0f;
    ...
    if (m_instance_buffer_dirty) {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     m_instance_data.size() * sizeof(ChunkInstanceData),
    ...
    m_shader->setMat4("projection",
                      m_camera->GetProjectionMatrix(1280, 720)); // Assume fixed aspect for loading
    ...
    ImGui::SetNextWindowPos(
        ImVec2(ImGui::GetIO().DisplaySize.x * 0.5f, ImGui::GetIO().DisplaySize.y * 0.85f),
        ImGuiCond_Always,
        ImVec2(0.5f, 0.5f));
    ...
    ImGui::Text("%d / %zu Chunks Dispatched", dispatched_chunks, total_chunks);
    ImGui::PopStyleVar();
