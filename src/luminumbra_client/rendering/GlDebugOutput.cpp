...
void GLAPIENTRY debug_message_router(GLenum source, GLenum type, GLuint id,
    ...
    // as an error regardless of the message type. RENDER-ONLY: we only log it.
    ...
    if (is_error) {
        ...
        LUMINUMBRA_CORE_ERROR("[GL][{}][{}][{}] id={} {}", source_str(source),
                              type_str(type), severity_str(severity), id,
                              message ? message : "");
        return;
    }
    ...
    switch (severity) {
        case GL_DEBUG_SEVERITY_NOTIFICATION:
            ...
            LUMINUMBRA_CORE_TRACE("[GL][{}][{}][NOTIFICATION] id={} {}", source_str(source),
                                  type_str(type), id, message ? message : "");
            break;
        default:  // LOW / MEDIUM warnings, perf tips, deprecation, portability.
            g_warnings.fetch_add(1, std::memory_order_relaxed);
            LUMINUMBRA_CORE_WARN("[GL][{}][{}][{}] id={} {}", source_str(source),
                                 type_str(type), severity_str(severity), id,
                                 message ? message : "");
            break;
    }
...
bool InstallGlDebugCallback() {
    ...
    if (glDebugMessageCallback == nullptr || glDebugMessageControl == nullptr) {
        LUMINUMBRA_CORE_WARN("LUMIN_GL_DEBUG set but KHR_debug entry points are unavailable; GL debug output disabled");
        return false;
    }
    ...
    LUMINUMBRA_CORE_INFO("GL KHR_debug output installed (synchronous{}); routing driver messages to the engine log",
                         verbose ? ", verbose" : ", notifications suppressed");
    return true;
#else
    (void)verbose;
    LUMINUMBRA_CORE_WARN("LUMIN_GL_DEBUG set but build lacks GL 4.3 KHR_debug headers; GL debug output disabled");
    return false;
#endif
}
