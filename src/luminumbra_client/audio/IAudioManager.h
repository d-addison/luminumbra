...
// AUDIO-05/AUDIO-10 (spec 021): the client mix-bus tree. Render/audio-side only
// (never sim). Hierarchy:
...
public:
    ...
    // One-shot playback with explicit bus routing (AUDIO-10). The default,
    ...
    virtual bool PlayOneShot(const AudioEventID& eventID, const glm::vec3& position,
                             BusId bus = BusId::Sfx) = 0;
    ...
    // to playing sounds and all future playback (AUDIO-05). Render-only.
    virtual void SetSfxVolume(float volume) = 0;
    // Generic bus gain [0,1] (AUDIO-10). Master/Music/Sfx forward to the dedicated
