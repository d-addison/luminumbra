...
AudioPropagationSystem::AudioPropagationSystem(Luminumbra::Systems::PhysicsSystem* physicsSystem)
    ...
    LUMINUMBRA_CORE_INFO("Audio Propagation System initialized with {} worker threads", num_threads);
}
...
AudioPropagationResult AudioPropagationSystem::CalculatePropagation(const glm::vec3& source, 
    ...
    if (distance <= max_distance && max_reflections > 0) {
        result.reflection_paths = CalculateReflectionPaths(source, listener, 
                                                         std::min(max_reflections, m_settings.max_reflection_depth));
    }
    ...
    float calculation_time = std::chrono::duration<float, std::milli>(end_time - start_time).count();
    UpdatePerformanceMetrics(calculation_time, false);
    
    return result;
}

void AudioPropagationSystem::CalculatePropagationAsync(const glm::vec3& source, 
                                                      const glm::vec3& listener,
...
AudioPropagationPath AudioPropagationSystem::CalculateDirectPath(const glm::vec3& source, const glm::vec3& listener) {
    ...
    if (m_physicsSystem) {
        ...
        if (ray_result.hit) {
            ...
            path.absorption_factor = AudioPropagationUtils::CalculateAirAbsorption(path.total_distance, 1000.0f);
            path.volume_multiplier = 1.0f - path.absorption_factor;
        }
    } else {
...
std::vector<AudioPropagationPath> AudioPropagationSystem::CalculateReflectionPaths(const glm::vec3& source, 
                                                                                  const glm::vec3& listener,
                                                                                  int max_bounces) {
    std::vector<AudioPropagationPath> paths;
    
    if (!m_physicsSystem) return paths;
    
    // Generate reflection points from physics system
    std::vector<glm::vec3> reflection_points = m_physicsSystem->calculate_audio_reflection_points(source, listener, max_bounces);
...
AudioPropagationSystem::EnvironmentalAnalysis AudioPropagationSystem::AnalyzeEnvironment(const glm::vec3& position, float analysis_radius) {
    ...
    if (analysis.is_enclosed) {
        ...
        analysis.rt60_estimate = CalculateRT60(analysis.room_volume, analysis.surface_area, analysis.avg_absorption);
    } else {
        // Outdoor environment
        analysis.room_volume = analysis_radius * analysis_radius * analysis_radius * 8.0f;
        analysis.surface_area = analysis_radius * analysis_radius * 6.0f * 4.0f;
...
float AudioPropagationSystem::CalculateRT60(float room_volume, float surface_area, float avg_absorption) {
    ...
    if (absorption_area < 0.01f) absorption_area = 0.01f; // Avoid division by zero
    
    return 0.161f * room_volume / absorption_area;
}

uint64_t AudioPropagationSystem::PropagationCache::HashPositions(const glm::vec3& source, const glm::vec3& listener, float tolerance) {
...
bool AudioPropagationSystem::ShouldUseCachedResult(const glm::vec3& source, const glm::vec3& listener) {
    uint64_t hash = m_cache.HashPositions(source, listener);
    std::lock_guard<std::mutex> lock(m_cache.cache_mutex);
    
    auto it = m_cache.entries.find(hash);
    if (it == m_cache.entries.end()) return false;
...
void AudioPropagationSystem::UpdatePerformanceMetrics(float calculation_time_ms, bool cache_hit) {
    if (cache_hit) {
        ...
        m_metrics.avg_calculation_time_ms = m_metrics.avg_calculation_time_ms * (1.0f - alpha) + 
                                           calculation_time_ms * alpha;
    }
}
...
void AudioPropagationSystem::WorkerThreadFunction() {
    while (!m_shutdown) {
        ...
        // Process task
        AudioPropagationResult result = CalculatePropagation(task.source, task.listener, task.max_distance);
