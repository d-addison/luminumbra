...
struct ClusterNode {
    ...
    ClusterNode() = default;
    ClusterNode(const AABB& bounds) : bounds(bounds) {}
};
...
public:
    ...
    // Source management
    void AddAudioSource(u32 event_handle, const glm::vec3& position, float volume, float min_distance, float max_distance);
    ...
    void SetClusteringEnabled(bool enabled) { m_clustering_enabled = enabled; }
    void SetOcclusionEnabled(bool enabled) { m_occlusion_enabled = enabled; }
    
    // Debug information
    int GetActiveSourceCount() const { return m_active_sources.size(); }
...
private:
    ...
    void ApplyLODProcessing(AudioSource& source, AudioDetailLevel detail, const glm::vec3& listener_pos);
