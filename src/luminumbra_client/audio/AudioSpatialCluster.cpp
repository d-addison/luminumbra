...
void AudioSpatialCluster::Update(const glm::vec3& listener_pos, float delta_time) {
    ...
    std::sort(m_sources_to_process.begin(), m_sources_to_process.end(),
...
void AudioSpatialCluster::AddAudioSource(u32 event_handle, const glm::vec3& position, float volume, float min_distance, float max_distance) {
...
void AudioSpatialCluster::BuildRecursive(ClusterNode* node, const std::vector<AudioSource*>& sources, int depth) {
    ...
    child_bounds[1] = ClusterNode::AABB(glm::vec3(center.x, node->bounds.min.y, node->bounds.min.z), glm::vec3(node->bounds.max.x, center.y, center.z));
    child_bounds[2] = ClusterNode::AABB(glm::vec3(node->bounds.min.x, center.y, node->bounds.min.z), glm::vec3(center.x, node->bounds.max.y, center.z));
    child_bounds[3] = ClusterNode::AABB(glm::vec3(center.x, center.y, node->bounds.min.z), glm::vec3(node->bounds.max.x, node->bounds.max.y, center.z));
    child_bounds[4] = ClusterNode::AABB(glm::vec3(node->bounds.min.x, node->bounds.min.y, center.z), glm::vec3(center.x, center.y, node->bounds.max.z));
    child_bounds[5] = ClusterNode::AABB(glm::vec3(center.x, node->bounds.min.y, center.z), glm::vec3(node->bounds.max.x, center.y, node->bounds.max.z));
    child_bounds[6] = ClusterNode::AABB(glm::vec3(node->bounds.min.x, center.y, center.z), glm::vec3(center.x, node->bounds.max.y, node->bounds.max.z));
    ...
    for (auto* source : sources) {
        int child_index = 0;
        if (source->position.x >= center.x) child_index += 1;
        if (source->position.y >= center.y) child_index += 2;
        if (source->position.z >= center.z) child_index += 4;
...
void AudioSpatialCluster::ProcessClusterRecursive(ClusterNode* node, const glm::vec3& listener_pos) {
    if (!node) return;
    ...
    if (node->is_leaf) {
        ...
        if (distance_to_cluster > CLUSTER_MERGE_DISTANCE) {
            ...
            float cluster_attenuation = 1.0f / (1.0f + cluster_distance * cluster_distance * 0.0001f);
...
AudioDetailLevel AudioSpatialCluster::CalculateDetailLevel(const AudioSource& source, const glm::vec3& listener_pos) {
    float distance = glm::distance(source.position, listener_pos);
...
void AudioSpatialCluster::ApplyLODProcessing(AudioSource& source, AudioDetailLevel detail, const glm::vec3& listener_pos) {
    float distance = glm::distance(source.position, listener_pos);
    
    switch (detail) {
        case AudioDetailLevel::Ultra:
            // Full processing: distance, occlusion, doppler, reverb
            source.last_calculated_attenuation = CalculateFullAttenuation(source, listener_pos, true, true, true);
            break;
            
        case AudioDetailLevel::High:
            // Full 3D processing: distance, occlusion, doppler
            source.last_calculated_attenuation = CalculateFullAttenuation(source, listener_pos, true, true, false);
            break;
            
        case AudioDetailLevel::Medium:
            // Distance + basic occlusion
            source.last_calculated_attenuation = CalculateFullAttenuation(source, listener_pos, true, false, false);
            break;
...
float AudioSpatialCluster::CalculateDistanceAttenuation(const AudioSource& source, float distance) {
    if (distance <= source.min_distance) return 1.0f;
    if (distance >= source.max_distance) return 0.0f;
    
    // Inverse square law with linear rolloff
    float normalized_distance = (distance - source.min_distance) / (source.max_distance - source.min_distance);
    return 1.0f - normalized_distance;
}

float AudioSpatialCluster::CalculateFullAttenuation(const AudioSource& source, const glm::vec3& listener_pos, bool include_occlusion, bool include_doppler, bool include_reverb) {
    float distance = glm::distance(source.position, listener_pos);
    ...
    // TODO: Add doppler and reverb processing when needed
    
    return attenuation;
}

void AudioSpatialCluster::CalculateBatchedOcclusion(const std::vector<AudioSource*>& sources, const glm::vec3& listener_pos) {
    ...
    if (m_physics_system) {
        ...
        for (auto* source : sources) {
            ...
            if (distance < MEDIUM_DISTANCE) {
                ...
                float fallback_occlusion = std::clamp(1.0f - (distance / MEDIUM_DISTANCE), 0.0f, 0.8f);
                
                // Queue the precise occlusion query
                float priority = 1.0f / (1.0f + distance * 0.01f); // Closer sources get higher priority
                
                int query_id = batch_queries.QueueRaycast(
                    source->position, 
                    listener_pos,
                    [this, source, listener_pos](const ::Luminumbra::Systems::PhysicsSystem::AudioRaycastResult& result) {
                        // Callback when the raycast completes
                        float occlusion = 0.0f;
                        if (result.hit) {
                            float total_distance = glm::distance(source->position, result.hit_point) + 
                            ...
                            occlusion = result.material_absorption * std::min(path_factor - 1.0f, 1.0f);
                        }
                        
                        // Apply the calculated occlusion to the source's attenuation
                        source->last_calculated_attenuation *= (1.0f - std::clamp(occlusion, 0.0f, 0.95f));
                    },
                    priority
                );
                ...
                source->last_calculated_attenuation *= (1.0f - fallback_occlusion * 0.3f); // Reduced impact until precise result
            }
        }
...
void AudioSpatialCluster::SetPhysicsRaycastCallback(std::function<bool(const glm::vec3&, const glm::vec3&, float&)> callback) {
