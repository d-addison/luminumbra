...
void UIDataStream::ShowDebugWindow(bool* p_open) {
    ...
    if (ImGui::Begin("Data Stream Debug", p_open)) {
        ...
        if (ImGui::BeginTable("StreamTable",
                              4,
            ...
            for (const auto& [id, binding] : m_bindings) {
                if (strlen(search_filter) == 0 || id.find(search_filter) != std::string::npos) {
                ...
                if (sort_specs->SpecsDirty) {
                    std::sort(sorted_bindings.begin(),
                              ...
                              [&](const auto& a, const auto& b) {
                                  if (sort_specs->Specs->SortDirection ==
...
void UIDataStream::LogStreamState() const {
    ...
    LUMINUMBRA_CORE_INFO("[UIDataStream] Average update time: {:.3f} ms",
                         m_stats.average_update_time_ms);

    for (const auto& [id, binding] : m_bindings) {
        std::string value = binding->GetValueAsString();
        bool changed = binding->HasChanged();
        LUMINUMBRA_CORE_INFO(
            "[UIDataStream] Stream '{}': {} (changed: {})", id, value, changed ? "yes" : "no");
    }
