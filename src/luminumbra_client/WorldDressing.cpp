// RENDER-20 (spec 021): the world-dressing placement loops, moved VERBATIM from
...
std::uint64_t InitialScatterRngState(float anchor_x, float anchor_z) {
    // T-I8 trees: the seed derivation from the spawn anchor (moved verbatim).
    return 0x9E3779B97F4A7C15ull ^
        (static_cast<std::uint64_t>(static_cast<std::int64_t>(anchor_x)) * 0xBF58476D1CE4E5B9ull) ^
        (static_cast<std::uint64_t>(static_cast<std::int64_t>(anchor_z)) * 0x94D049BB133111EBull);
}
...
std::vector<TreePlacement> ComputeTreePlacements(const WorldDressingParams& params,
    ...
    auto frand = [&rng]() { return ScatterFrand(rng); };
    // I8 BF1-grove tree-scatter knobs (render-only client decoration, never
    ...
    // Track-B LOD + frustum cull), so the full reach fills densely without
    // per-tree cost.
    ...
    for (float dz = -reach; dz <= reach && placed < kMaxInstances; dz += cell) {
        for (float dx = -reach; dx <= reach && placed < kMaxInstances; dx += cell) {
            ...
            if (h < cbs.water_level(x, zc) + 1.0f) continue;
            ...
            // DENSITY CONVENTION (FOLIAGE-01 root cause — this probe shipped
            ...
            const float slope = glm::max(
                glm::max(std::abs(cbs.terrain_height(x + hs, zc) - h), std::abs(cbs.terrain_height(x - hs, zc) - h)),
                glm::max(std::abs(cbs.terrain_height(x, zc + hs) - h), std::abs(cbs.terrain_height(x, zc - hs) - h)));
            ...
            // I9-FOLIAGE: bake deterministic GENETIC + MATURITY size variation so
...
std::vector<RockPlacement> ComputeRockPlacements(const WorldDressingParams& params,
    ...
    // ROCK FORMATIONS (worldgen-richness slice 1): DENSER on steep terrain
    // (scree / rocky outcrops), a sparse baseline of boulders on open ground,
    // and rare large sentinels. RENDER-ONLY (never hashed); the frand() stream
    ...
    if (params.rock_palette_count <= 0) return out;
    auto frand = [&rng]() { return ScatterFrand(rng); };
    ...
    for (float gz = -rReach; gz <= rReach && rocksPlaced < rCap; gz += rCell) {
        for (float gx = -rReach; gx <= rReach && rocksPlaced < rCap; gx += rCell) {
            ...
            if (hC <= cbs.water_level(rx, rz) + 0.3f) continue; // not underwater (sea or lake)
...
std::vector<BushPlacement> ComputeBushPlacements(const WorldDressingParams& params,
    ...
    // SHRUB/BUSH LAYER (spec 003 FR-A2): scatter on flatter, vegetated ground —
    ...
    for (float gz = -bReach; gz <= bReach && bushPlaced < bCap; gz += bCell) {
        for (float gx = -bReach; gx <= bReach && bushPlaced < bCap; gx += bCell) {
            ...
            if (hC <= cbs.water_level(bx, bz) + 0.5f) continue; // not in water
