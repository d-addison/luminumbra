...
// I9-FOLIAGE Phase 5A: data-driven SPECIES REGISTRY. The engine stays GENERIC — a species is DATA
// (data/common/foliage/species/*.json): a render-archetype string, annual/perennial + lifespan, and
// per-gene [lo,hi] ranges over the existing PlantGene schema. The registry loads those templates and
// can deterministically SAMPLE a PlantGenomeComponent from a species' ranges (seeded RNG), so a
// farm/spawn picks "wheat" or "oak" by id and gets an in-bounds, heritable genome. No species-specific
// logic lives in the engine — only this loader + the data.
//
// DETERMINISM: SampleGenome draws from a caller-supplied seeded DeterministicRng (no wall-clock); the
// sampled genes are clamped to [0,1]. Loading is pure file I/O; the sim/world_hash is unaffected until
// a sampled plant is actually spawned (plants are opt-in via PlantTag).
...
[[nodiscard]] inline int SpeciesGeneIndex(const std::string& name) {
    ...
    if (name == "GrowthRate") return static_cast<int>(G::GrowthRate);
    if (name == "MaxScale") return static_cast<int>(G::MaxScale);
    if (name == "DroughtTolerance") return static_cast<int>(G::DroughtTolerance);
    if (name == "ColdTolerance") return static_cast<int>(G::ColdTolerance);
    if (name == "HeatTolerance") return static_cast<int>(G::HeatTolerance);
    if (name == "LeafDensity") return static_cast<int>(G::LeafDensity);
    if (name == "Yield") return static_cast<int>(G::Yield);
    if (name == "Hardiness") return static_cast<int>(G::Hardiness);
    return -1;
}
...
[[nodiscard]] inline bool ParseSpeciesTemplate(const nlohmann::json& j, SpeciesTemplate& out,
    ...
    if (j.contains("genes") && j.at("genes").is_object()) {
        for (auto it = j.at("genes").begin(); it != j.at("genes").end(); ++it) {
            ...
            if (idx < 0 || !it.value().is_array() || it.value().size() < 2) continue;
...
public:
    ...
    bool AddFromJsonText(const std::string& text, std::string& err) {
        try {
            SpeciesTemplate t;
            if (!ParseSpeciesTemplate(nlohmann::json::parse(text), t, err)) return false;
    ...
    [[nodiscard]] const std::vector<SpeciesTemplate>& all() const { return m_species; }

    // Deterministically sample a genome from a species' per-gene ranges using a seeded RNG.
    [[nodiscard]] static Comp::PlantGenomeComponent SampleGenome(
        const SpeciesTemplate& t, luminumbra::core::DeterministicRng& rng) {
