...
// Spec 024 (AETHER-06): the STATEFUL energy-field layer — the persistent half of
...
//
// TRUTH DOMAIN (FR-024-2): integer fixed-point ONLY. Cells are uint16 raw units
...
//
// WORLD-ANCHORED SPARSE PAGES (FR-024-9): unlike the re-derivable field's
...
//               (FFF-52/WATER-17 ordering law); clipped raw is returned for the
...
//
// SUB-HASH (FR-024-3): CanonicalBytes() emits the "aether_state:v1:" tagged
...
//
// PERSISTENCE (FR-024-4): SerializeRecord() NORMALIZES first (catch-up decay
...
inline constexpr const char* kEnergyFieldStateSchema =
    "luminumbra.fields.energy_field_state.v1";
...
public:
    // channels >= 1. v1 ships single-channel; AETHER-08 constructs with 2
    ...
    // Phase 1 of the deposit path: gather. Applied (sorted, saturating) by the
    ...
    void QueueDeposit(std::uint64_t emitter_id, int cx, int cz, int channel,
    ...
    // Persistence (FR-024-4). Serialize normalizes (catch-up to save_tick's
    ...
    [[nodiscard]] std::uint64_t fires_completed() const noexcept { return m_fires_completed; }
    [[nodiscard]] std::uint64_t next_fire_tick() const noexcept { return m_next_fire_tick; }
...
private:
    ...
    [[nodiscard]] static std::uint16_t DecaySteps(std::uint16_t v,
    ...
    // rebases across the load-resets-tick-to-zero boundary (FR-024-4).
