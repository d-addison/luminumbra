...
// Reserved seed-stream offset for the photo-filters track (registry slot). The
// module is PURE (no rng), so this is recorded for collision-avoidance only and is
// intentionally never consumed.
inline constexpr std::uint64_t kPhotoFilterSeedOffset = 25ull;
