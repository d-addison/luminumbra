...
// Track game.codex_entry — the per-SPECIES metadata layer of the photography codex
// (pillar G). Where PhotoCodex records what the player has CAPTURED (per-species
...
public:
    ...
    // iteration order).
    const std::vector<SpeciesInfo>& all() const { return entries_; }
