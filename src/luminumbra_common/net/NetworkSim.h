...
// I10-NET: deterministic NETWORK-CONDITION simulator over the ILockstepTransport seam.
...
// identically. Latency/jitter are measured in Tick()s (engine ticks), never wall-clock.
...
class NetworkSim {
public:
    explicit NetworkSim(NetworkConditions cond) : m_cond(cond) {}

    void Tick() { ++m_tick; }
    [[nodiscard]] std::uint64_t now() const { return m_tick; }

    [[nodiscard]] ILockstepTransport& A() { return m_a; }
    [[nodiscard]] ILockstepTransport& B() { return m_b; }
...
private:
    ...
    bool Send(bool isA, const std::vector<std::uint8_t>& f, FrameDelivery d) {
        ...
        if (!selfOpen || !peerOpen) return false;
        ...
        if (m_cond.jitter_ticks > 0) {
            ...
            delay += static_cast<std::int64_t>(r % (2u * static_cast<unsigned>(m_cond.jitter_ticks) + 1u)) -
                     m_cond.jitter_ticks;
        }
        if (delay < 0) delay = 0;
        PendingFrame pf;
    ...
    bool Recv(bool isA, std::vector<std::uint8_t>& out) {
        ...
        for (int i = 0; i < static_cast<int>(q.size()); ++i) {
            if (q[i].deliver_tick > m_tick) continue;
    ...
    [[nodiscard]] bool PeerOpen(bool isA) const { return isA ? m_bOpen : m_aOpen; }
    void CloseEnd(bool isA) { (isA ? m_aOpen : m_bOpen) = false; }

    class Ep final : public ILockstepTransport {
    public:
        Ep(NetworkSim* sim, bool isA) : m_sim(sim), m_isA(isA) {}
        ...
        [[nodiscard]] bool IsPeerConnected() const override { return m_sim->PeerOpen(m_isA); }
        void Close() override { m_sim->CloseEnd(m_isA); }
