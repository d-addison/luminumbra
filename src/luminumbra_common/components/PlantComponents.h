...
// I9-FOLIAGE: Living-World Foliage pillar — the SIMULATION-SIDE plant state.
//
// LOAD-BEARING RULE (see memory living-world-foliage-pillar): procedural plant
// GEOMETRY is visual-only (client/render, baked per genome+stage); the SIM truth
// is THIS small, integer/fixed-point, deterministic state. Growth advances on the
...
// ai/Evolution.h) so breeding = the SAME blend-crossover + Gaussian-mutate
...
struct PlantGenomeComponent {
    ...
    float gene(PlantGene g) const { return genes[static_cast<std::size_t>(g)]; }
};
...
enum class PlantStage : std::uint8_t {
    Seed = 0, Sprout, Juvenile, Mature, Flowering, Fruiting, Count
};
