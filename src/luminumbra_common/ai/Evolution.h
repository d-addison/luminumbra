...
// T-I9-AI: deterministic real-valued genetic-algorithm operators — the evolution
// pillar's compute core (spec emergent-ecology R-P5). A genome is a small vector
...
// World_hash-neutral until a ReproductionSystem wires it into the sim.
...
inline void GaussianMutate(std::vector<float>& genome, const std::vector<GeneBound>& bounds,
                           float sigma_frac, luminumbra::core::DeterministicRng& rng) {
...
inline std::vector<float> BlendCrossover(const std::vector<float>& a, const std::vector<float>& b,
...
inline std::size_t TournamentSelect(const std::vector<float>& fitness, int k,
                                    luminumbra::core::DeterministicRng& rng) {
...
inline std::vector<std::vector<float>> EvolveGeneration(
    const std::vector<std::vector<float>>& pop, const std::vector<float>& fitness,
    const std::vector<GeneBound>& bounds, float sigma_frac, int elitism, int tournament_k,
    luminumbra::core::DeterministicRng& rng) {
    ...
    for (std::size_t e = 0; e < elite; ++e) next.push_back(pop[order[e]]);
