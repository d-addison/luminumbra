...
// T-I9-AI: creature PERCEPTION primitives — vision cones + hearing — the
...
[[nodiscard]] inline bool InVisionCone(float ox, float oz, float fx, float fz,
                                       float cos_half_fov, float range, float tx, float tz) {
    const float dx = tx - ox;
    const float dz = tz - oz;
    const float dist2 = dx * dx + dz * dz;
    if (dist2 > range * range) return false; // out of range
    if (dist2 < 1e-8f) return true;          // coincident
...
[[nodiscard]] inline float PerceivedLoudness(float dist, float source_loudness,
                                             float hearing_range) {
...
[[nodiscard]] inline float PitchSensitivity(const HearingProfile& ear, float pitch) {
    ...
    if (d < 0.0f) d = -d;
    if (ear.bandwidth <= 0.0f) return d == 0.0f ? 1.0f : 0.0f;
...
[[nodiscard]] inline float HeardLoudness(float dist, float source_loudness,
                                         float source_pitch, const HearingProfile& ear) {
