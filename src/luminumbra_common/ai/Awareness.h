...
// T-I9-AI: perception FUSION — the awareness / detection-meter state machine that
// turns a fused sensory signal (vision + hearing + smell) into a graduated
// awareness state plus a remembered last-known position (spec emergent-ecology
// P1). Detection is NOT binary: a meter accumulates while a target is sensed and
...
[[nodiscard]] inline Awareness UpdateAwareness(const Awareness& prev, float signal, float tx,
                                               float tz, float dt, const AwarenessParams& p) {
    Awareness a = prev;
    ...
    if (a.meter < 0.0f) a.meter = 0.0f;
    if (a.meter > 1.0f) a.meter = 1.0f;
