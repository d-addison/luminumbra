...
// Fixed-rate simulation clock (T-I3-4). Accumulates variable frame dt and
// converts it into fixed simulation ticks. The canonical shipped rate is
// 30 Hz (design-decisions.md §1); the rate parameter exists for tests only.
//
...
public:
    ...
    [[nodiscard]] double tick_rate_hz() const noexcept { return tick_rate_hz_; }
    [[nodiscard]] std::uint32_t max_catch_up_ticks() const noexcept { return max_catch_up_ticks_; }
    ...
    [[nodiscard]] std::uint64_t dropped_frame_count() const noexcept { return dropped_frame_count_; }
