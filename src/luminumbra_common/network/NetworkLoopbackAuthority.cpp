...
constexpr const char* kRuntimeJoinLeaveServerModeContract =
    "server_ticks_continue_while_clients_join_and_leave";
...
constexpr const char* kRemoteAvatarReplicationContract =
    "server_snapshot_drives_client_remote_avatar_transforms";
constexpr const char* kRemoteAvatarInterpolationContract =
    "client_render_behind_interpolated_remote_poses";
...
void WriteJsonUInt(std::ostream& out,
                   const std::string& key,
...
void WriteJsonInt(std::ostream& out,
                  const std::string& key,
                  const int value,
                  const bool comma = true) {
    out << "    \"" << key << "\": " << value;
...
void WriteJsonBool(std::ostream& out,
                   const std::string& key,
                   const bool value,
                   const bool comma = true) {
...
void WritePortArray(std::ostream& out,
                    const std::string& key,
...
NetworkLoopbackState ApplyAcceptedInput(NetworkLoopbackState state,
                                        const NetworkLoopbackInput& input) {
...
std::string BuildAuthoritativeChecksum(const std::vector<NetworkLoopbackDecision>& decisions,
...
bool SameState(const NetworkLoopbackState& left, const NetworkLoopbackState& right) {
    return left.tick == right.tick && left.authoritativeRevision == right.authoritativeRevision &&
           left.positionXMm == right.positionXMm && left.positionYMm == right.positionYMm;
}
...
std::vector<NetworkLoopbackCheck> BuildChecks(const NetworkLoopbackConvergenceReport& report) {
    return {
        {"network loopback authority API is declared",
         report.serializer == kSerializer && report.validationApi == kValidationApi},
        ...
        {"loopback convergence reaches deterministic state",
         report.converged &&
             SameState(report.finalAuthoritativeState, report.reconciledClientState)},
...
std::vector<NetworkMultiClientAcceptCheck>
BuildMultiClientAcceptChecks(const NetworkMultiClientAcceptReport& report) {
    return {
        {"multi-client accept API is declared",
...
std::vector<NetworkRuntimeJoinLeaveCheck>
BuildRuntimeJoinLeaveChecks(const NetworkRuntimeJoinLeaveReport& report) {
    return {
        ...
        {"runtime accept ports reuse the deterministic multi-client mapping",
         report.deterministicPortMapping},
    };
}

std::vector<NetworkRemoteAvatarRenderCheck>
BuildRemoteAvatarRenderChecks(const NetworkRemoteAvatarRenderReport& report) {
    return {
        {"remote-avatar render API is declared",
        ...
        {"remote avatars are submitted through the skinned render pass",
         report.remoteAvatarsRendered},
        {"local player id is excluded from the remote set",
...
bool AvatarPosePositionsAreFiniteMillimeters(
...
bool PortsMatchExpectedMapping(const std::vector<std::uint16_t>& ports,
...
BuildNetworkLoopbackConvergenceFixture(const std::string& buildPreset) {
    ...
    for (const auto& input : inputs) {
        const bool accepted = input.clientId == kAuthoritativeClientId &&
                              !input.clientAuthorityClaim && input.tick > authoritativeState.tick;
    ...
    report.unauthorizedAuthorityClaimRejected =
        report.rejectedFrameCount == 1 &&
    ...
    report.authoritativeChecksum =
        BuildAuthoritativeChecksum(report.decisions, report.finalAuthoritativeState);
...
std::string
SerializeNetworkLoopbackConvergenceJson(const NetworkLoopbackConvergenceReport& report) {
    std::ostringstream out;
    out << "{\n";
    ...
    WriteJsonBool(
        out, "unauthorized_authority_claim_rejected", report.unauthorizedAuthorityClaimRejected);
    ...
    WriteJsonInt(
        out, "prediction_error_before_reconcile_mm", report.predictionErrorBeforeReconcileMm);
    WriteJsonInt(
        out, "prediction_error_after_reconcile_mm", report.predictionErrorAfterReconcileMm);
    ...
    for (std::size_t i = 0; i < report.decisions.size(); ++i) {
        ...
        out << "      \"authoritative_revision\": "
            << decision.authoritativeState.authoritativeRevision << ",\n";
    ...
    for (std::size_t i = 0; i < report.checks.size(); ++i) {
        ...
        out << "    { \"name\": \"" << EscapeJson(check.name)
            << "\", \"passed\": " << BoolLiteral(check.passed) << " }";
...
bool NetworkLoopbackAuthorityMeetsBaseline(const NetworkLoopbackConvergenceReport& report) {
    ...
    if (report.serializer != kSerializer || report.validationApi != kValidationApi ||
        report.artifactWriter != kArtifactWriter) {
        return false;
    }
    ...
    if (report.transport != kTransport || report.simulation != kSimulation ||
        report.authoritativeClientId != kAuthoritativeClientId) {
        return false;
    }
    if (report.submittedFrameCount < 6 || report.acceptedFrameCount < 5 ||
        report.rejectedFrameCount < 1) {
        return false;
    }
    if (!report.unauthorizedAuthorityClaimRejected || !report.clientPredictionReconciled ||
        !report.converged) {
        return false;
    }
    if (report.predictionErrorBeforeReconcileMm <= 0 ||
        report.predictionErrorAfterReconcileMm != 0) {
        return false;
    }
    ...
    if (!report.decisions[2].clientId.empty() &&
        (report.decisions[2].clientId != "client-beta" || report.decisions[2].accepted ||
         report.decisions[2].reason != "client_authority_claim_rejected")) {
        return false;
    }
...
bool TryNetworkMultiClientAcceptPortForClient(const std::uint16_t basePort,
...
BuildNetworkMultiClientAcceptFixture(std::uint32_t expectedClientCount,
                                     const std::uint16_t basePort) {
    ...
    for (std::uint32_t clientId = report.firstClientId; clientId <= report.lastClientId;
         ++clientId) {
        std::uint16_t port = 0;
    ...
    report.uniquePlayerIds =
        report.firstClientId == 1u && report.lastClientId == expectedClientCount;
    report.deterministicPortMapping =
...
bool NetworkMultiClientAcceptMeetsBaseline(const NetworkMultiClientAcceptReport& report) {
    if (report.schema != kMultiClientSchema || report.source != kSourcePath ||
        report.header != kHeaderPath || report.portMappingApi != kMultiClientPortMappingApi ||
        report.validationApi != kMultiClientValidationApi ||
        report.artifactWriter != kMultiClientArtifactWriter ||
        report.acceptContract != kMultiClientAcceptContract) {
        return false;
    }
    if (report.expectedClientCount < 2u || report.firstClientId != 1u ||
        report.lastClientId != report.expectedClientCount) {
        return false;
    }
    if (!report.tcpAcceptsAllExpectedClients || !report.udpAcceptsAllExpectedClients ||
        !report.uniquePlayerIds || !report.deterministicPortMapping) {
        return false;
    }
    if (!PortsMatchExpectedMapping(
            report.tcpAcceptPorts, report.basePort, report.expectedClientCount) ||
        !PortsMatchExpectedMapping(
            report.udpAcceptPorts, report.basePort, report.expectedClientCount)) {
        return false;
    }
...
NetworkRuntimeJoinLeaveReport BuildNetworkRuntimeJoinLeaveFixture(std::uint32_t expectedClientCount,
                                                                  const std::uint16_t basePort,
    ...
    for (std::uint32_t clientId = 1; clientId <= expectedClientCount; ++clientId) {
        ...
        client.finalAckedSnapshotSeq =
            client.leftAtTick != 0u ? client.leftAtTick - 1u : ticksExecuted;
    ...
    const bool has_expected_clients =
        report.clients.size() == static_cast<std::size_t>(expectedClientCount);
    report.lateJoinAccepted =
        has_expected_clients && !report.clients.empty() && report.clients[0].joinedAtTick > 1u;
    report.leaveDoesNotStopHost = has_expected_clients && !report.clients.empty() &&
    ...
    report.stablePlayerIds = has_expected_clients && report.clients.size() >= 2u &&
                             report.clients[0].clientId == 1u && report.clients[1].clientId == 2u &&
                             report.clients[0].acceptPort != report.clients[1].acceptPort;
    report.deterministicPortMapping =
        PortsMatchExpectedMapping(ports, basePort, expectedClientCount);
...
std::string SerializeNetworkRuntimeJoinLeaveJson(const NetworkRuntimeJoinLeaveReport& report) {
    ...
    for (std::size_t i = 0; i < report.events.size(); ++i) {
        ...
        out << "    {\"tick\": " << event.tick << ", \"client_id\": " << event.clientId
...
bool NetworkRuntimeJoinLeaveMeetsBaseline(const NetworkRuntimeJoinLeaveReport& report) {
    if (report.schema != kRuntimeJoinLeaveSchema || report.source != kSourcePath ||
        report.header != kHeaderPath || report.portMappingApi != kMultiClientPortMappingApi ||
        report.validationApi != kRuntimeJoinLeaveValidationApi ||
        report.artifactWriter != kRuntimeJoinLeaveArtifactWriter ||
        report.serverModeContract != kRuntimeJoinLeaveServerModeContract) {
    ...
    if (report.expectedClientCount < 2u || report.ticksExecuted < 8u ||
        report.clients.size() != static_cast<std::size_t>(report.expectedClientCount) ||
        report.events.size() < 6u) {
        return false;
    }
    if (!report.emptyServerTicksBeforeJoin || !report.lateJoinAccepted ||
        !report.leaveDoesNotStopHost || !report.hostRunsAfterLastLeave || !report.stablePlayerIds ||
        !report.deterministicPortMapping) {
        return false;
    }
...
bool WriteNetworkRuntimeJoinLeaveArtifact(const std::string& path,
    ...
    const auto report =
        BuildNetworkRuntimeJoinLeaveFixture(expectedClientCount, basePort, ticksExecuted);
    std::ofstream out(path, std::ios::binary);
    if (!out) {
        return false;
    }
...
BuildNetworkRemoteAvatarRenderReport(const std::uint32_t localClientId,
                                     const std::vector<NetworkRemoteAvatarRenderPose>& poses,
    ...
    report.remoteAvatarsInterpolated =
        report.remoteAvatarCount >= expectedAvatarCount - 1u && all_remote_interpolated;
    report.remoteAvatarsRendered = renderedAvatarCount >= expectedAvatarCount &&
                                   skinnedDraws >= expectedAvatarCount &&
                                   skinnedIndicesDrawn > 0u && all_remote_rendered;
    report.localAvatarExcludedFromRemoteSet = local_present && !local_marked_remote &&
                                              report.remoteAvatarCount == expectedAvatarCount - 1u;
    report.deterministicClientOrdering =
        unique_client_ids && AvatarPosesAreOrderedByClientId(report.poses);
...
NetworkRemoteAvatarRenderReport
BuildNetworkRemoteAvatarRenderFixture(std::uint32_t expectedAvatarCount,
                                      const std::uint32_t renderedAvatarCount,
                                      const std::uint32_t skinnedDraws,
                                      const std::uint32_t skinnedIndicesDrawn) {
    if (expectedAvatarCount < 2u) {
    ...
    return BuildNetworkRemoteAvatarRenderReport(
        1u, poses, expectedAvatarCount, 3u, renderedAvatarCount, skinnedDraws, skinnedIndicesDrawn);
}
...
std::string SerializeNetworkRemoteAvatarRenderJson(const NetworkRemoteAvatarRenderReport& report) {
    ...
    WriteJsonBool(
        out, "local_avatar_excluded_from_remote_set", report.localAvatarExcludedFromRemoteSet);
    WriteJsonBool(out, "deterministic_client_ordering", report.deterministicClientOrdering, false);
    out << "  },\n";
    out << "  \"poses\": [\n";
    for (std::size_t i = 0; i < report.poses.size(); ++i) {
        const NetworkRemoteAvatarRenderPose& pose = report.poses[i];
        out << "    {\"client_id\": " << pose.clientId << ", \"server_tick\": " << pose.serverTick
...
bool NetworkRemoteAvatarRenderMeetsBaseline(const NetworkRemoteAvatarRenderReport& report) {
    if (report.schema != kRemoteAvatarRenderSchema || report.source != kSourcePath ||
        report.header != kHeaderPath || report.builderApi != kRemoteAvatarRenderBuilderApi ||
        report.validationApi != kRemoteAvatarRenderValidationApi ||
        report.artifactWriter != kRemoteAvatarRenderArtifactWriter ||
        report.replicationContract != kRemoteAvatarReplicationContract ||
        report.interpolationContract != kRemoteAvatarInterpolationContract ||
    ...
    if (report.expectedAvatarCount < 2u || report.remoteAvatarCount < 1u ||
        report.poses.size() < static_cast<std::size_t>(report.expectedAvatarCount)) {
        return false;
    }
    if (!report.serverSnapshotsReceived || !report.remoteAvatarsInterpolated ||
        !report.remoteAvatarsRendered || !report.localAvatarExcludedFromRemoteSet ||
        !report.deterministicClientOrdering) {
        return false;
    }
...
bool WriteNetworkRemoteAvatarRenderFixtureArtifact(const std::string& path,
    ...
    const auto report = BuildNetworkRemoteAvatarRenderFixture(
        expectedAvatarCount, renderedAvatarCount, skinnedDraws, skinnedIndicesDrawn);
