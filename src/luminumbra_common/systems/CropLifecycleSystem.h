...
// I9-FOLIAGE Phase 2 (FR-G germination): CropLifecycleSystem — advances ripe plants through
...
// canonical roster stays byte-identical. SEED OFFSET REGISTRY: germination CLAIMS +36 (season +37 is
// reserved for future seasonal env variation; not consumed yet — the season phase is tick%year, no RNG).
...
inline CropLifecycleStats RunCropLifecycleOnTick(entt::registry& reg, std::uint64_t tick,
                                                 std::uint64_t world_seed = 0) {
    namespace Comp = ::Luminumbra::Components;
    namespace dm = ::Luminumbra::DeterministicMath;
    CropLifecycleStats stats;

    auto view = reg.view<Comp::CropLifecycleComponent, Comp::PlantGrowthComponent,
                         const Comp::PlantGenomeComponent, const Comp::TransformComponent>();
    std::vector<entt::entity> ents;
    for (auto e : view) ents.push_back(e);
    if (ents.empty()) return stats;  // opt-in: no participants -> byte-identical no-op
    ...
    for (auto e : ents) {
        ...
        if (g.stage < kFruiting) { lc.ripe_ticks = 0; continue; }  // still growing
        ++stats.ripening;
        ++lc.ripe_ticks;
        if (lc.ripe_ticks < lc.lifespan_ticks) continue;  // ripening, not yet senescent
        ...
        if (const auto* pc = reg.try_get<const Comp::PollinationComponent>(e); pc && pc->pollinated) {
        ...
        luminumbra::core::DeterministicRng rng = luminumbra::core::DeterministicRng::seeded(
            ...
            static_cast<std::uint64_t>(entt::to_integral(e)), tick);
        ...
        const ::Luminumbra::Vec3 childPos(tf.position.x + rad * dm::Cos(ang), tf.position.y,
                                          tf.position.z + rad * dm::Sin(ang));
