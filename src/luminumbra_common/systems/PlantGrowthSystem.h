...
// I9-FOLIAGE: Living-World Foliage pillar — phenotype expansion, the deterministic
...
// taken; plants claim +15 — record in design-decisions when wired to world_hash).
...
inline float fabs_det(float v) { return v < 0.0f ? -v : v; } // libm-free abs
...
struct PlantGrowthStats { int updated = 0; int advanced = 0; };
...
inline PlantGrowthStats RunPlantGrowthSystemOnTick(
    entt::registry& reg, std::uint64_t current_tick, const EnvSampler& sample_env) {
    PlantGrowthStats stats;
    auto view = reg.view<Comp::PlantTag, Comp::PlantGrowthComponent,
...
inline Comp::PlantGenomeComponent RandomGenome(luminumbra::core::DeterministicRng& rng) {
    ...
    for (auto& gene : g.genes) gene = rng.next_unit();
    return g;
}
...
inline Comp::PlantGenomeComponent BreedPlants(
    const Comp::PlantGenomeComponent& a, const Comp::PlantGenomeComponent& b,
    luminumbra::core::DeterministicRng& rng, float mutation_frac = 0.06f) {
    ...
    const std::vector<luminumbra::ai::GeneBound> bounds(child.size(), luminumbra::ai::GeneBound{0.0f, 1.0f});
    luminumbra::ai::GaussianMutate(child, bounds, mutation_frac, rng);
    Comp::PlantGenomeComponent out;
    for (std::size_t i = 0; i < out.genes.size() && i < child.size(); ++i) out.genes[i] = child[i];
    return out;
}
