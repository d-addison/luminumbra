...
// the game grows (the camera produces a PhotoShot; this scores it). Keeping the
...
// Reserved seed-stream offset for the photo-scoring track (registry: wind+11,
...
inline float ScoreComposition(const PhotoShot& shot) {
    ...
    for (std::size_t i = 1; i < shot.subjects.size(); ++i) {
        if (shot.subjects[i].size > shot.subjects[main].size) main = i;
    }
...
inline float ScoreLighting(const PhotoShot& shot) {
    ...
    const float expo = Clamp01(1.0f - kExposureFalloff * AbsF(Clamp01(shot.exposure) - kIdealExposure));
...
inline float ScoreFocus(const PhotoShot& shot) {
    ...
    for (const auto& s : shot.subjects) {
        if (s.size > main_size) main_size = s.size;
    }
...
inline float ScoreRarity(const PhotoShot& shot) {
    ...
    for (const auto& s : shot.subjects) {
        ...
        for (std::size_t k = 0; k < ids.size(); ++k) {
            if (ids[k] == s.species_id) { ++counts[k]; found = true; break; }
        }
        if (!found) { ids.push_back(s.species_id); counts.push_back(1); }
    }
