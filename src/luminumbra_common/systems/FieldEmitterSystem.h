...
// Spec 024 FR-024-5 (AETHER-07): FieldEmitterSystem — the deposit GATHER pass
// feeding the stateful energy layer. Phase 1 of the two-phase deposit path:
...
// saturation (FFF-52/WATER-17 ordering law), so neither registry iteration
...
inline FieldEmitterGatherStats GatherFieldEmitterDeposits(
    entt::registry& registry, luminumbra::fields::EnergyFieldState& field) {
    ...
    std::vector<entt::entity> emitters;
    {
        ...
        for (auto e : view) emitters.push_back(e);
    ...
    for (auto e : emitters) {
        ...
        if (emitter.rate_raw_per_tick == 0u) continue;  // zero-default = inert
