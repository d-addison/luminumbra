...
std::uint64_t OrderedEventBus::publish(std::uint64_t tick, std::string topic, std::string payload, std::int32_t lane)
{
...
EventBusReplayResult replay_eventbus_order_fixture()
{
    ...
    result.passed = describe_event_order(result.delivered_events) == expected_order && !result.checksum.empty();
    return result;
}
