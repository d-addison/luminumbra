// Track game.codex_entry — the per-SPECIES metadata + rarity layer of the photography
// codex (pillar G), pairing with PhotoCodex. These tests pin the catalogue model:
...
// Registering a species then Find()ing it returns the stored metadata verbatim.
...
TEST(SpeciesCodex, RunEqualsReplayOrderIndependent) {
    ...
    SpeciesRegistry a;
    for (const auto& s : forward) a.Register(s);
    SpeciesRegistry b;
    for (const auto& s : shuffled) b.Register(s);
    ...
    for (std::size_t i = 0; i < a.all().size(); ++i) {
        ...
        EXPECT_EQ(a.RarityWeight(a.all()[i].species_id),
                  b.RarityWeight(b.all()[i].species_id));
    }
