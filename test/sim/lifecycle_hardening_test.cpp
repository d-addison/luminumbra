...
struct LifeSnap {
    ...
    std::vector<float> f;
    bool operator==(const LifeSnap& o) const { return u == o.u && f == o.f; }
};

LifeSnap snapshotGenomes(entt::registry& r) {
    LifeSnap s;
    std::vector<entt::entity> es;
    auto v = r.view<Comp::CreatureGenomeComponent, Comp::CreatureComponent,
                    Comp::TransformComponent>();
    for (auto e : v) es.push_back(e);
...
TEST(ReproHardening, PopulationIsBounded) {
    ...
    for (int i = 0; i < 3; ++i) spawnFertile(r, 0.1f * i, 0.0f, /*female=*/true);
    for (int i = 0; i < 3; ++i) spawnFertile(r, 0.1f * i, 0.2f, /*female=*/false);
...
TEST(ReproHardening, RunEqualsReplay) {
    auto build = [](entt::registry& r) {
        for (int i = 0; i < 3; ++i) spawnFertile(r, 0.1f * i, 0.0f, true);
        for (int i = 0; i < 3; ++i) spawnFertile(r, 0.1f * i, 0.2f, false);
    };
...
TEST(LifespanHardening, OrderIndependentDeaths) {
    auto buildA = [](entt::registry& r) {
        auto a = r.create(); r.emplace<Comp::MortalComponent>(a).lifespan_ticks = 2;
        auto b = r.create(); r.emplace<Comp::MortalComponent>(b).lifespan_ticks = 100;
    };
    // Same entities, different declared order — but death depends on lifespan only.
    entt::registry r1, r2;
    buildA(r1);
    // r2: create the long-lived one first.
    auto b2 = r2.create(); r2.emplace<Comp::MortalComponent>(b2).lifespan_ticks = 100;
    auto a2 = r2.create(); r2.emplace<Comp::MortalComponent>(a2).lifespan_ticks = 2;
...
TEST(DecompHardening, LiveDoesNotDecay) {
    ...
    for (int t = 0; t < 20; ++t) luminumbra::ai::RunDecompositionOnTick(r, t);
...
TEST(DecompHardening, MilliContractMonotonicLandsExact) {
    ...
    EXPECT_EQ(summedPerTick,
              static_cast<std::uint64_t>(luminumbra::ai::kNutrientPerCorpseMilli));
}
...
TEST(DecompHardening, FinishedIsLatchedStable) {
    ...
    auto& dc = r.emplace<Comp::DecayComponent>(e);
    dc.decay_duration = 3;
    for (int t = 0; t < 3; ++t) luminumbra::ai::RunDecompositionOnTick(r, t);
...
TEST(DecompHardening, RunEqualsReplay) {
    ...
    auto run = [](entt::registry& r) {
        for (int t = 0; t < 40; ++t) luminumbra::ai::RunDecompositionOnTick(r, t);
        std::vector<std::uint32_t> out;
        std::vector<entt::entity> es;
        for (auto e : r.view<Comp::DecayComponent>()) es.push_back(e);
        std::sort(es.begin(), es.end(), [](entt::entity a, entt::entity b) {
            return entt::to_integral(a) < entt::to_integral(b);
        });
        for (auto e : es) {
...
TEST(CircadianHardening, DayWrapContinuity) {
    ...
    EXPECT_NEAR(luminumbra::ai::CircadianActivity(1.25f, false),
                luminumbra::ai::CircadianActivity(0.25f, false), 1e-4f);
    EXPECT_NEAR(luminumbra::ai::CircadianActivity(-0.1f, false),
                luminumbra::ai::CircadianActivity(0.9f, false), 1e-4f);
}
...
TEST(CircadianHardening, OrderIndependentActivity) {
    ...
    auto d1 = r1.create(); r1.emplace<Comp::CircadianComponent>(d1).nocturnal = 0;
    auto n1 = r1.create(); r1.emplace<Comp::CircadianComponent>(n1).nocturnal = 1;
    // r2: nocturnal then diurnal.
    auto n2 = r2.create(); r2.emplace<Comp::CircadianComponent>(n2).nocturnal = 1;
    auto d2 = r2.create(); r2.emplace<Comp::CircadianComponent>(d2).nocturnal = 0;
...
TEST(CircadianHardening, RunEqualsReplay) {
    ...
    auto run = [](entt::registry& r) {
        ...
        for (float t : times) {
            ...
            std::vector<entt::entity> es;
            for (auto e : r.view<Comp::CircadianComponent>()) es.push_back(e);
            std::sort(es.begin(), es.end(), [](entt::entity a, entt::entity b) {
                return entt::to_integral(a) < entt::to_integral(b);
            });
            for (auto e : es) out.push_back(r.get<Comp::CircadianComponent>(e).activity);
        }
        return out;
    };
    entt::registry a, b;
...
TEST(WildlifeHardening, BiomassClampedNonNegative) {
    ...
    for (int i = 0; i < 10; ++i) spawnHerbivore(r, 0.0f, 0.0f, 0.5f);  // big herd on it
...
TEST(WildlifeHardening, RegrowthCappedAtOne) {
    ...
    for (int t = 0; t < 50; ++t) luminumbra::ai::RunWildlifeFoliageOnTick(r, t);
...
TEST(WildlifeHardening, RunEqualsReplay) {
    auto build = [](entt::registry& r) {
        for (int i = 0; i < 6; ++i) spawnGrazePlant(r, 0.4f * i, 0.0f, 1.0f);
        for (int i = 0; i < 4; ++i) spawnHerbivore(r, 0.4f * i, 0.3f, 0.4f);
    };
    auto run = [](entt::registry& r) {
        ...
        for (auto e : r.view<Comp::GrazeableComponent>()) ps.push_back(e);
        std::sort(ps.begin(), ps.end(), [](entt::entity a, entt::entity b) {
            return entt::to_integral(a) < entt::to_integral(b);
        });
        for (auto e : ps) out.push_back(r.get<Comp::GrazeableComponent>(e).biomass);
        std::vector<entt::entity> cs;
        for (auto e : r.view<Comp::CreatureComponent>()) cs.push_back(e);
        std::sort(cs.begin(), cs.end(), [](entt::entity a, entt::entity b) {
            return entt::to_integral(a) < entt::to_integral(b);
        });
        for (auto e : cs) out.push_back(r.get<Comp::CreatureComponent>(e).hunger);
        return out;
    };
    entt::registry a, b;
    build(a);
