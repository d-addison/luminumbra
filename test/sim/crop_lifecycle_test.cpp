// I9-FOLIAGE Phase 2 (FR-G germination): CropLifecycleSystem — ripe plants senesce and reseed.
...
entt::entity spawnRipe(entt::registry& r, bool perennial, std::uint32_t lifespan,
                       const Luminumbra::Vec3& pos, const C::PlantGenomeComponent& genome) {
    auto e = r.create();
...
std::size_t plantCount(entt::registry& r) { return r.view<C::PlantTag>().size(); }
...
TEST(CropLifecycle, GrowingPlantDoesNotSenesce) {
    entt::registry r;
    auto e = spawnRipe(r, /*perennial=*/false, /*lifespan=*/3, Luminumbra::Vec3(0, 0, 0), RandomGenome(1));
    r.get<C::PlantGrowthComponent>(e).stage = static_cast<std::uint8_t>(C::PlantStage::Sprout);  // not terminal
    for (std::uint64_t t = 1; t <= 10; ++t) F::RunCropLifecycleOnTick(r, t);
...
TEST(CropLifecycle, AnnualDiesAndReseeds) {
    entt::registry r;
    auto parent = spawnRipe(r, /*perennial=*/false, /*lifespan=*/3, Luminumbra::Vec3(0, 0, 0), RandomGenome(2));
    for (std::uint64_t t = 1; t <= 4; ++t) F::RunCropLifecycleOnTick(r, t);
    ...
    for (auto e : r.view<C::PlantTag>()) {
        EXPECT_EQ(r.get<C::PlantGrowthComponent>(e).stage, static_cast<std::uint8_t>(C::PlantStage::Seed));
...
TEST(CropLifecycle, PerennialResetsAndRegrowsAndReseeds) {
    entt::registry r;
    auto parent = spawnRipe(r, /*perennial=*/true, /*lifespan=*/1, Luminumbra::Vec3(0, 0, 0), RandomGenome(3));
    ...
    EXPECT_EQ(r.get<C::PlantGrowthComponent>(parent).stage,
              static_cast<std::uint8_t>(C::PlantStage::Juvenile)) << "reset to a vegetative stage";
...
TEST(CropLifecycle, GerminatesFromPollinationCross) {
    entt::registry r;
    auto parent = spawnRipe(r, /*perennial=*/false, /*lifespan=*/1, Luminumbra::Vec3(0, 0, 0), RandomGenome(4));
...
TEST(CropLifecycle, DeterministicReplay) {
    auto run = [] {
        entt::registry r;
        for (int i = 0; i < 6; ++i)
            spawnRipe(r, /*perennial=*/(i % 2 == 0), /*lifespan=*/2,
                      Luminumbra::Vec3(static_cast<float>(i * 4), 0, 0), RandomGenome(static_cast<std::uint64_t>(i)));
        for (std::uint64_t t = 1; t <= 30; ++t) F::RunCropLifecycleOnTick(r, t);
