// gate-populated-world-replay (T009 + T006): unit coverage for the id-ordered
...
//
// T006 — additivity guard: appending the `|ecology:` term to a composite changes
...
void PopulateRoster(entt::registry& r) {
    {
        ...
        pk.coord_x = 0.3f; pk.coord_z = -0.4f; pk.in_pack = 1;
    }
    for (int i = 0; i < 2; ++i) {
        auto e = r.create();
        ...
        cr.wish_x = 0.5f * i; cr.wish_z = -0.25f * i;
        auto& gn = r.emplace<Comp::CreatureGenomeComponent>(e);
        gn.female = (i % 2 == 0); gn.age_ticks = 100u; gn.generation = 0u;
        r.emplace<Comp::AlarmComponent>(e).level = 0.2f * i;
    }
}

// T009: the hash is byte-exact across two identically-built rosters.
...
// T009: the hash commits to the id-ordered STATE projection, not iteration order.
...
TEST(EcologyHash, OrderIndependentOverIdSortedState) {
    auto build = [](bool predator_first) {
        ...
        auto mk = [&](bool predator, float x) {
            ...
            auto& cr = r.emplace<Comp::CreatureComponent>(e);
            cr.is_predator = predator; cr.hunger = predator ? 0.9f : 0.1f;
        };
        ...
        if (predator_first) { mk(true, -6.0f); mk(false, 4.0f); }
        else                { mk(true, -6.0f); mk(false, 4.0f); }
        return luminumbra::ai::ComputeEcologySubHash(r);
    };
    EXPECT_EQ(build(true), build(false));
}

// T006: the additivity guard, asserted on the StableChecksum algebra. Appending
...
TEST(EcologyHash, AppendingEmptyEcologyAddsOnlyTheSuffix) {
    ...
    const std::string prefix =
        "chunkHASH|wind:WINDH|weather:WEATHERH|aether:AETHERH|scents:SCENTH";
    ...
    const std::string composite_post =
        P::StableChecksum(prefix + "|ecology:" + ecology_neutral);
