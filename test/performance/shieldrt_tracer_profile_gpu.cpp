...
// ---------------------------------------------------------------------------
// Hidden GL 4.5 core context (matches render_smoke_test's harness). Skips the
// test when no context can be created (headless CI).
// ---------------------------------------------------------------------------
...
public:
    HiddenGlContext() {
        ...
        m_window = glfwCreateWindow(64, 64, "shieldrt_tracer_profile_gpu", nullptr, nullptr);
        if (!m_window) {
...
GLuint CompileComputeProgram(const char* src, std::string& error) {
...
GLuint MakeStorageBuffer(GLsizeiptr bytes, const void* data) {
    GLuint buf = 0;
    glGenBuffers(1, &buf);
...
void SetIntArray(GLuint prog, const char* name, const std::int32_t* data, int count) {
...
bool IsSoftwareRenderer(const std::string& renderer) {
