// T-I6 P1 (multiplayer): the server-authoritative player avatar primitives.
...
// (byte-identical to the pre-P1 headless `entities` lane). The avatar->streaming
...
float horiz_len(const Vec3& v) { return std::sqrt(v.x * v.x + v.z * v.z); }
...
TEST(PlayerAvatar, SpawnOffsetsAreDistinctAndFanOut) {
    ...
    for (std::uint32_t id = 0; id < 20; ++id) offs.push_back(DeterministicAvatarSpawnOffset(id));
...
TEST(PlayerAvatar, EmptyAvatarsYieldEmptySnapshot) {
    const std::string empty_builtin =
        Luminumbra::Ecs::SerializeEntityRegistrySnapshotJson(Luminumbra::Ecs::EntityRegistrySnapshot{});
    const std::string empty_from_avatars =
        Luminumbra::Ecs::SerializeEntityRegistrySnapshotJson(BuildAvatarEntitySnapshot({}));
    // Byte-identical to the pre-P1 empty lane -> default entities sub-hash unchanged.
...
TEST(PlayerAvatar, SnapshotIsDeterministicOrderedAndNonEmpty) {
    ...
    PlayerAvatar a2; a2.player_id = 2; a2.position = Vec3(10.0f, 5.0f, -3.0f); a2.facing = 1.5f;
    PlayerAvatar a0; a0.player_id = 0; a0.position = Vec3(0.0f, 4.0f, 0.0f);
    PlayerAvatar a1; a1.player_id = 1; a1.position = Vec3(-2.5f, 4.2f, 7.1f); a1.velocity = Vec3(1.0f, 0.0f, 0.0f);
    avatars = {a2, a0, a1};

    const std::string s1 = Luminumbra::Ecs::SerializeEntityRegistrySnapshotJson(BuildAvatarEntitySnapshot(avatars));
    // Reorder the input -> identical serialization (order-independent / sorted).
    std::vector<PlayerAvatar> reordered = {a0, a1, a2};
    const std::string s2 = Luminumbra::Ecs::SerializeEntityRegistrySnapshotJson(BuildAvatarEntitySnapshot(reordered));
    EXPECT_EQ(s1, s2);
