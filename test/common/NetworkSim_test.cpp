// I10-NET: deterministic network-condition simulator tests (latency / jitter / loss),
...
std::vector<std::uint8_t> frame(std::uint8_t id) { return {id}; }
...
TEST(NetworkSim, ReliableNeverDropped) {
    ...
    for (int i = 0; i < 10; ++i) sim.A().SendFrame(frame(static_cast<std::uint8_t>(i)),
                                                   FrameDelivery::Reliable);
    int got = 0;
    std::vector<std::uint8_t> out;
    while (sim.B().TryReceiveFrame(out)) ++got;
    EXPECT_EQ(got, 10);
}
...
TEST(NetworkSim, UnreliableDropsSomeDeterministically) {
    auto run = [] {
        ...
        for (int i = 0; i < 30; ++i) sim.A().SendFrame(frame(static_cast<std::uint8_t>(i)),
                                                       FrameDelivery::Unreliable);
        std::vector<std::uint8_t> survived, out;
        while (sim.B().TryReceiveFrame(out)) survived.push_back(out[0]);
        return survived;
    };
...
TEST(NetworkSim, DeterministicUnderJitter) {
    auto run = [] {
        ...
        for (int t = 0; t < 8; ++t) {
            while (sim.B().TryReceiveFrame(out)) order.push_back(out[0]);
            sim.Tick();
        }
        return order;
    };
