...
TEST(IsolationConfig, ParseSingleAndMultiLayer) {
    ...
    EXPECT_EQ(ParseIsolationLayers("terrain,water,foliage"),
              L::Terrain | L::Water | L::Foliage);
...
TEST(IsolationConfig, BackdropModeNameRoundTrips) {
    for (auto m : {BackdropMode::Scene, BackdropMode::Void, BackdropMode::Greenscreen,
                   BackdropMode::Checker, BackdropMode::Transparent}) {
