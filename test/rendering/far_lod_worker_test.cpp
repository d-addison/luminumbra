...
enum class SharedPlaneAxis { X, Z };

std::vector<PlaneSegmentBits> SharedPlaneSegments(
    const FarLodRegionMesh& mesh,
    int region_x,
    int region_z,
    ...
    const float local_plane = world_plane -
        (axis == SharedPlaneAxis::X ? origin_x : origin_z);
...
std::size_t CountIndexedWorldVertex(
    const FarLodRegionMesh& mesh,
    int region_x,
    int region_z,
    float world_x,
    ...
    float world_z,
    u32 material) {
    const float local_x = world_x -
        static_cast<float>(region_x * kFarLodRegionSizeMeters);
    const float local_z = world_z -
        static_cast<float>(region_z * kFarLodRegionSizeMeters);
    std::set<u32> matches;
...
std::size_t CountIndexedWorldPosition(
    const FarLodRegionMesh& mesh,
    int region_x,
    int region_z,
    float world_x,
...
bool PlaneSegmentsContainHeight(
    const std::vector<PlaneSegmentBits>& segments,
    float world_y) {
...
bool SetBoundaryRampedAuthority(
    Chunk& chunk,
    float shared_face_y,
    float interior_y,
    u8 material) {
    const int side = CHUNK_SIZE_X + 1;
    const std::size_t expected_samples = static_cast<std::size_t>(side) * side * side;
    if (chunk.sdf_data.size() != expected_samples) return false;
    chunk.material_data.assign(expected_samples, material);
    for (int z = 0; z <= CHUNK_SIZE_Z; ++z) {
        for (int y = 0; y <= CHUNK_SIZE_Y; ++y) {
            for (int x = 0; x <= CHUNK_SIZE_X; ++x) {
...
CrossRegionBuildResult BuildCrossRegionBoundary(
    FarLodTier tier,
    ...
    const auto build_home_and_save = [&]() -> bool {
        home = BuildFarLodWorkerTile(
            world, *home_snapshot, tier, home_rx, home_rz, save.path);
        if (!home.ok) {
    ...
    const auto build_target = [&]() -> bool {
        target = BuildFarLodWorkerTile(
            world, *target_snapshot, tier, target_rx, target_rz, save.path);
        if (!target.ok) {
    ...
    const FarLodTile pristine_target = BuildPristineFarLodTile(
        world, tier, target_rx, target_rz, target_snapshot->params_hash);
    ...
    const float world_plane = static_cast<float>((target_region_axis > home_region_axis
        ? target_region_axis : home_region_axis) * kFarLodRegionSizeMeters);
    result.home_plane_segments = SharedPlaneSegments(
        home.mesh, home_rx, home_rz, axis, world_plane);
    result.target_plane_segments = SharedPlaneSegments(
        target.mesh, target_rx, target_rz, axis, world_plane);
    result.ok = true;
    return result;
}
...
TEST(FarLodWorker, AuthorityRevisionInvalidationTargetsHomeEdgesAndCorners) {
    ...
    for (const InvalidationCase& test_case : cases) {
        SCOPED_TRACE(::testing::Message()
            << test_case.coords.x << "," << test_case.coords.z);
        const TerrainGenParams params = FlatParams();
        SHIELD_WorldSystem world(nullptr, nullptr, params, 1337);
...
TEST(FarLodWorker, CrossRegionBoundariesAreBitIdenticalAndHomeOwnedForBothTiers) {
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        for (const BoundaryCase& boundary : boundaries) {
            ...
            const CrossRegionBuildResult home_first = BuildCrossRegionBoundary(
                tier, boundary.authority_coords,
                boundary.home_rx, boundary.home_rz,
                boundary.target_rx, boundary.target_rz, boundary.axis, true);
            const CrossRegionBuildResult target_first = BuildCrossRegionBoundary(
                tier, boundary.authority_coords,
                boundary.home_rx, boundary.home_rz,
                boundary.target_rx, boundary.target_rz, boundary.axis, false);
            ...
            EXPECT_NE(home_first.target_mesh_hash,
                      home_first.target_pristine_mesh_hash)
                << "foreign authority did not alter target-region geometry";
            EXPECT_NE(target_first.target_mesh_hash,
                      target_first.target_pristine_mesh_hash)
                << "foreign authority did not alter target-region geometry";
...
TEST(FarLodWorker, RegionCornerAuthorityBuildsAllFourRegionsDeterministically) {
    ...
    const std::array<CornerCase, 2> corners{{
        {"positive-max-corner", IVec3(31, 0, 31),
         {{{0, 0}, {1, 0}, {0, 1}, {1, 1}}}},
        {"negative-min-corner", IVec3(-32, 0, -32),
         {{{-1, -1}, {-2, -1}, {-1, -2}, {-2, -2}}}},
    }};
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        for (const CornerCase& corner : corners) {
            SCOPED_TRACE(::testing::Message()
                << corner.name << " tier=" << static_cast<int>(tier));
            const TerrainGenParams params = FlatParams();
            SHIELD_WorldSystem world(nullptr, nullptr, params, 1337);
            ...
            const auto build_order = [&](const std::array<int, 4>& order) {
                ...
                for (const int index : order) {
                    ...
                    EXPECT_TRUE(snapshot);
                    if (snapshot) {
                        outcomes[static_cast<std::size_t>(index)] = BuildFarLodWorkerTile(
                            world, *snapshot, tier, rx, rz, {});
                    }
                }
                return outcomes;
            };
            ...
            for (std::size_t i = 0; i < forward.size(); ++i) {
                ...
                const FarLodTile pristine = BuildPristineFarLodTile(
                    world, tier, rx, rz, forward[i].tile.params_hash);
                FarLodRegionMesh pristine_mesh;
                MarchingCubes::GenerateFarLodRegionMesh(pristine, pristine_mesh);
            ...
            const auto compare_plane = [&](std::size_t a, std::size_t b,
                                           SharedPlaneAxis axis) {
                ...
                const float plane = static_cast<float>(
                    std::max(region_a, region_b) * kFarLodRegionSizeMeters);
                const auto a_segments = SharedPlaneSegments(
                    forward[a].mesh, arx, arz, axis, plane);
                const auto b_segments = SharedPlaneSegments(
                    forward[b].mesh, brx, brz, axis, plane);
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        for (const bool material_mismatch : {false, true}) {
            SCOPED_TRACE(::testing::Message()
                ...
                << " mismatch=" << (material_mismatch ? "material" : "density"));
            const TerrainGenParams params = FlatParams();
            SHIELD_WorldSystem world(nullptr, nullptr, params, 1337);
            ...
            TempSaveDir save;
            const FarLodTile sentinel = BuildPristineFarLodTile(
                world, tier, 0, 0, snapshot->params_hash);
            ...
            const auto outcome = BuildFarLodWorkerTile(
                world, *snapshot, tier, 0, 0, save.path);
            EXPECT_FALSE(outcome.ok);
...
TEST(FarLodWorker, LiveBoundarySnapshotSupersedesPersistedNeighborAuthority) {
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        ...
        const auto old_home = BuildFarLodWorkerTile(
            old_world, *old_home_snapshot, tier, 0, 0, save.path);
        ...
        const auto old_target = BuildFarLodWorkerTile(
            old_world, *old_target_snapshot, tier, 1, 0, save.path);
        ...
        const auto mixed = BuildFarLodWorkerTile(
            new_world, *new_target_snapshot, tier, 1, 0, save.path);
        const auto live_only = BuildFarLodWorkerTile(
            new_world, *new_target_snapshot, tier, 1, 0, {});
...
TEST(FarLodWorker, FullSdfBoundaryAuthoritySupersedesLegacyNeighborMaterial) {
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        ...
        FarLodTile legacy = BuildPristineFarLodTile(
            world, tier, 0, 0, ComputeTerrainParamsHash(params, 1337));
        legacy.edited = true;
        ...
        const auto mixed = BuildFarLodWorkerTile(
            world, *snapshot, tier, 1, 0, save.path);
        const auto live_only = BuildFarLodWorkerTile(
            world, *snapshot, tier, 1, 0, {});
        ASSERT_TRUE(mixed.ok) << mixed.error;
        ASSERT_TRUE(live_only.ok) << live_only.error;
        EXPECT_EQ(HashMesh(mixed.mesh), HashMesh(live_only.mesh));
        EXPECT_EQ(std::count_if(
            mixed.mesh.vertices.begin(), mixed.mesh.vertices.end(),
...
TEST(FarLodWorker, LegacyHaloUsesIndexedGeometryAndFinalAuthorityPrecedence) {
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        for (const bool home_first : {true, false}) {
            ...
            FarLodTile legacy = BuildPristineFarLodTile(
                world, tier, 1, 0, params_hash);
            legacy.edited = true;
            legacy.legacy_surface_authority = true;
            const auto set_legacy = [&](int world_x, int world_z, float height,
                                        u8 material, u8 flags) {
                const std::size_t x = static_cast<std::size_t>(
                    (world_x - kFarLodRegionSizeMeters) / step);
            ...
            const std::size_t probe_index = set_legacy(
                kProbeWorldX, kProbeWorldZ, kLegacyHeight, kLegacyMaterial,
                kFarLodSampleFlagEdited | kFarLodSampleFlagWater);
            const std::size_t face_index = set_legacy(
                kFarLodRegionSizeMeters, kProbeWorldZ,
                kSupersededFaceHeight, kSupersededFaceMaterial,
                kFarLodSampleFlagEdited | kFarLodSampleFlagWater);
            std::vector<std::string> errors;
            ASSERT_TRUE(FarLodStore(save.path).save_tile(legacy, &errors));
            ...
            EXPECT_GT(CountIndexedWorldVertex(
                target.mesh, 1, 0, static_cast<float>(kProbeWorldX),
                kLegacyHeight, static_cast<float>(kProbeWorldZ),
                kLegacyMaterial), 0u)
                << "the exact legacy witness must be referenced by an index";
            EXPECT_EQ(CountIndexedWorldVertex(
                target.mesh, 1, 0,
                static_cast<float>(kFarLodRegionSizeMeters),
                kSupersededFaceHeight, static_cast<float>(kProbeWorldZ),
                kSupersededFaceMaterial), 0u);
            EXPECT_EQ(CountIndexedWorldPosition(
                target.mesh, 1, 0,
                static_cast<float>(kFarLodRegionSizeMeters),
                kSupersededFaceHeight, static_cast<float>(kProbeWorldZ)), 0u)
                << "supersession must remove the legacy plane independent of material";
            EXPECT_GT(CountIndexedWorldVertex(
                target.mesh, 1, 0,
                static_cast<float>(kFarLodRegionSizeMeters), 7.0f,
                static_cast<float>(kProbeWorldZ), kSdfMaterial), 0u);
            ...
            errors.clear();
            ASSERT_TRUE(FarLodStore(save.path).load_tile(
                tier, 0, 0, params_hash, persisted_home, &errors));
            ...
            EXPECT_EQ(persisted_target.height_q[probe_index],
                      QuantizeFarLodHeight(kLegacyHeight));
            ...
            const auto replay_home_snapshot =
                replay_world.capture_far_lod_sdf_snapshot(0, 0);
            const auto replay_target_snapshot =
                replay_world.capture_far_lod_sdf_snapshot(1, 0);
            ...
            EXPECT_EQ(CountIndexedWorldPosition(
                ...
                kSupersededFaceHeight, static_cast<float>(kProbeWorldZ)), 0u);
            EXPECT_GT(CountIndexedWorldVertex(
                replay_target.mesh, 1, 0,
                static_cast<float>(kProbeWorldX), kLegacyHeight,
                static_cast<float>(kProbeWorldZ), kLegacyMaterial), 0u);
            EXPECT_EQ(replay_target.tile.flags[face_index] &
                      kFarLodSampleFlagEdited, 0u);
            const auto replay_home_segments = SharedPlaneSegments(
                replay_home.mesh, 0, 0, SharedPlaneAxis::X,
                static_cast<float>(kFarLodRegionSizeMeters));
            const auto replay_target_segments = SharedPlaneSegments(
                replay_target.mesh, 1, 0, SharedPlaneAxis::X,
                static_cast<float>(kFarLodRegionSizeMeters));
            ...
            errors.clear();
            ASSERT_TRUE(FarLodStore(save.path).save_tile(
                replay_target.tile, &errors));
            ...
            EXPECT_EQ(persisted_promoted_target.flags[probe_index] &
                      kFarLodSampleFlagEdited, kFarLodSampleFlagEdited);
            EXPECT_EQ(persisted_promoted_target.flags[face_index] &
                      kFarLodSampleFlagEdited, 0u);
            ...
            ASSERT_TRUE(SetBoundaryRampedAuthority(
                *replacement, 7.0f, 11.0f, kSdfMaterial));
            ASSERT_TRUE(world.adopt_streamed_chunk(replacement));
            const auto replacement_snapshot =
                world.capture_far_lod_sdf_snapshot(1, 0);
            ASSERT_TRUE(replacement_snapshot);
            const auto superseded = BuildFarLodWorkerTile(
                world, *replacement_snapshot, tier, 1, 0, save.path);
            ASSERT_TRUE(superseded.ok) << superseded.error;
            EXPECT_EQ(superseded.tile.flags[probe_index] &
                      kFarLodSampleFlagEdited, 0u);
            EXPECT_EQ(CountIndexedWorldVertex(
                superseded.mesh, 1, 0, static_cast<float>(kProbeWorldX),
                kLegacyHeight, static_cast<float>(kProbeWorldZ),
                kLegacyMaterial), 0u);
            EXPECT_EQ(CountIndexedWorldPosition(
                superseded.mesh, 1, 0, static_cast<float>(kProbeWorldX),
                kLegacyHeight, static_cast<float>(kProbeWorldZ)), 0u)
                << "real chunk 32 must remove the old height for every material";
            EXPECT_GT(CountIndexedWorldVertex(
                superseded.mesh, 1, 0, static_cast<float>(kProbeWorldX),
                11.0f, static_cast<float>(kProbeWorldZ), kSdfMaterial), 0u);

            errors.clear();
            ASSERT_TRUE(FarLodStore(save.path).save_tile(
                superseded.tile, &errors));
            ...
            EXPECT_EQ(persisted_superseded_target.flags[probe_index] &
                      kFarLodSampleFlagEdited, 0u);
            EXPECT_EQ(persisted_superseded_target.flags[face_index] &
                      kFarLodSampleFlagEdited, 0u);
...
TEST(FarLodWorker, ForeignLegacyMaxFaceAppliesExactSavedWaterBit) {
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        for (const bool saved_water : {true, false}) {
            ...
            TempSaveDir save;
            TerrainGenParams params = FlatParams();
            params.height_offset = saved_water
                ? SEA_LEVEL + 24.0f : SEA_LEVEL - 24.0f;
            SHIELD_WorldSystem world(nullptr, nullptr, params, 1337);
            const int step = FarLodSampleStepMeters(tier);
            const u64 params_hash = ComputeTerrainParamsHash(params, 1337);

            FarLodTile foreign = BuildPristineFarLodTile(
                world, tier, 1, 0, params_hash);
            foreign.edited = true;
            ...
            const std::size_t home_index =
                ...
                static_cast<std::size_t>(kWorldZ / step) *
                    foreign.samples_per_side;
            const FarLodTile pristine_home = BuildPristineFarLodTile(
                world, tier, 0, 0, params_hash);
            EXPECT_EQ(
                (pristine_home.flags[home_index] &
                    kFarLodSampleFlagWater) != 0u,
                !saved_water)
            ...
            foreign.flags[foreign_index] = static_cast<u8>(
                ...
                (saved_water ? kFarLodSampleFlagWater : 0u));
            std::vector<std::string> errors;
            ...
            ASSERT_TRUE(outcome.ok) << outcome.error;
            EXPECT_GT(CountIndexedWorldVertex(
                outcome.mesh, 0, 0, static_cast<float>(kWorldX),
                kLegacyHeight, static_cast<float>(kWorldZ), kLegacyMaterial), 0u);

            EXPECT_EQ(
                (outcome.tile.flags[home_index] &
                    kFarLodSampleFlagWater) != 0u,
                saved_water)
                << "the saved bit must set or clear, never merely OR";
            EXPECT_EQ(outcome.tile.flags[home_index] &
                      kFarLodSampleFlagEdited, 0u)
...
TEST(FarLodWorker, NegativeLegacyMaxFaceUsesFloorOwnedCell) {
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        ...
        FarLodTile foreign = BuildPristineFarLodTile(
            world, tier, kForeignRegionX, kHomeRegionZ, params_hash);
        foreign.edited = true;
        foreign.legacy_surface_authority = true;
        const int foreign_origin_z =
            kHomeRegionZ * kFarLodRegionSizeMeters;
        const std::size_t foreign_index =
            static_cast<std::size_t>((kWorldZ - foreign_origin_z) / step) *
            foreign.samples_per_side;
        foreign.height_q[foreign_index] = QuantizeFarLodHeight(kLegacyHeight);
        foreign.material[foreign_index] = kLegacyMaterial;
        foreign.flags[foreign_index] =
            kFarLodSampleFlagEdited | kFarLodSampleFlagWater;
        std::vector<std::string> errors;
        ASSERT_TRUE(FarLodStore(save.path).save_tile(foreign, &errors));
        ...
        ASSERT_TRUE(SetPlanarAuthority(*authority, 7.0f, kSdfMaterial));
        ASSERT_TRUE(world.adopt_streamed_chunk(authority));
        const auto snapshot = world.capture_far_lod_sdf_snapshot(
            kHomeRegionX, kHomeRegionZ);
        ASSERT_TRUE(snapshot);
        const auto outcome = BuildFarLodWorkerTile(
            world, *snapshot, tier, kHomeRegionX, kHomeRegionZ, save.path);
        ASSERT_TRUE(outcome.ok) << outcome.error;
        EXPECT_GT(CountIndexedWorldVertex(
            outcome.mesh, kHomeRegionX, kHomeRegionZ,
            static_cast<float>(kWorldX), kLegacyHeight,
            static_cast<float>(kWorldZ), kLegacyMaterial), 0u);
        EXPECT_GT(CountIndexedWorldPosition(
            outcome.mesh, kHomeRegionX, kHomeRegionZ,
            static_cast<float>(kWorldX), kLegacyHeight,
            static_cast<float>(kWorldZ)), 0u);
...
TEST(FarLodWorker, DurableChunkTruthRepairsOldFarWithoutAStreamedSnapshot) {
    ...
    for (const FarLodTier tier : {FarLodTier::F1, FarLodTier::F2}) {
        ...
        ASSERT_TRUE(old_target_snapshot);
        const auto old_home = BuildFarLodWorkerTile(
            old_world, *old_home_snapshot, tier, 0, 0, {});
        const auto old_target = BuildFarLodWorkerTile(
            old_world, *old_target_snapshot, tier, 1, 0, {});
        ASSERT_TRUE(old_home.ok) << old_home.error;
        ASSERT_TRUE(old_target.ok) << old_target.error;
        std::vector<std::string> errors;
        ASSERT_TRUE(FarLodStore(save.path).save_tile(old_home.tile, &errors));
        ...
        const auto recovery_home_snapshot =
            recovery_world.capture_far_lod_sdf_snapshot(0, 0);
        const auto recovery_target_snapshot =
            recovery_world.capture_far_lod_sdf_snapshot(1, 0);
        ...
        const auto recovered_home = BuildFarLodWorkerTile(
            recovery_world, *recovery_home_snapshot, tier, 0, 0, save.path);
        const auto recovered_target = BuildFarLodWorkerTile(
            recovery_world, *recovery_target_snapshot, tier, 1, 0, save.path);
        ...
        const auto reference_home_snapshot =
            reference_world.capture_far_lod_sdf_snapshot(0, 0);
        const auto reference_target_snapshot =
            reference_world.capture_far_lod_sdf_snapshot(1, 0);
        ASSERT_TRUE(reference_home_snapshot);
        ASSERT_TRUE(reference_target_snapshot);
        const auto reference_home = BuildFarLodWorkerTile(
            reference_world, *reference_home_snapshot, tier, 0, 0, {});
        const auto reference_target = BuildFarLodWorkerTile(
            reference_world, *reference_target_snapshot, tier, 1, 0, {});
