// T-I9-AI E3 (FR-3): ant-trail FORAGING (Deneubourg double-bridge) — deposit-on-both-trips into two
// ScentField channels, follow the opposite channel, pick up at food / deliver at the nest. Pure grid
...
entt::entity spawnForager(entt::registry& r, std::int32_t home_x, std::int32_t home_z) {
    ...
    f.home_x = home_x; f.home_z = home_z;
    f.cell_x = home_x; f.cell_z = home_z;
    return e;
}
entt::entity spawnFood(entt::registry& r, std::int32_t x, std::int32_t z, std::int32_t amount = 1000000) {
    auto e = r.create();
    auto& f = r.emplace<Comp::FoodSourceComponent>(e);
    f.cell_x = x; f.cell_z = z; f.amount = amount;
    return e;
}
...
TEST(Foraging, RoundTripDelivers) {
    ...
    spawnFood(r, 4, 0);
    for (int t = 0; t < 30; ++t) RunForagingOnTick(r, f, {});
    EXPECT_GE(r.get<Comp::ForagerComponent>(ant).deliveries, 1u) << "should complete >=1 round trip";
}
...
TEST(Foraging, DepositsOnBothTrips) {
    ...
    spawnForager(r, 0, 0);
    spawnFood(r, 4, 0);
    for (int t = 0; t < 20; ++t) RunForagingOnTick(r, f, {});
    double home = 0.0, food = 0.0;
    for (int x = 0; x <= 4; ++x) { home += f.Sample(kHomeTrailChannel, x, 0); food += f.Sample(kFoodTrailChannel, x, 0); }
...
TEST(Foraging, Deterministic) {
    auto run = [] {
        ...
        auto ant = spawnForager(r, 0, 0);
        spawnFood(r, 6, 0);
        for (int t = 0; t < 60; ++t) { RunForagingOnTick(r, f, {}); f.Step(0.2, 1, 0.05); }
        double acc = 0.0;
        for (int c = 2; c < 4; ++c)
            for (int z = 0; z < 5; ++z)
                for (int x = 0; x < 24; ++x) acc += f.Sample(c, x, z);
        return std::pair<std::uint32_t, double>(r.get<Comp::ForagerComponent>(ant).deliveries, acc);
    };
    EXPECT_EQ(run(), run());
}

// Shortest-path throughput (the basis of double-bridge selection): a shorter route is completed more
// often per unit time, so it delivers more food than a longer one over the same number of ticks.
TEST(Foraging, ShorterRouteDeliversMore) {
    auto routeDeliveries = [](std::int32_t foodX, int ticks) {
        ...
        auto ant = spawnForager(r, 0, 0);
        spawnFood(r, foodX, 0);
        for (int t = 0; t < ticks; ++t) { RunForagingOnTick(r, f, {}); f.Step(0.2, 1, 0.05); }
...
TEST(Foraging, FollowsTrailWhenGoalNeutral) {
    ...
    for (int i = 0; i < 6; ++i) f.Step(0.25, 2, 0.0);
    ForagingParams p; p.goal_weight = 0.0;  // pure trail following
    const std::int32_t x0 = r.get<Comp::ForagerComponent>(ant).cell_x;
    for (int t = 0; t < 4; ++t) RunForagingOnTick(r, f, p);
    EXPECT_GT(r.get<Comp::ForagerComponent>(ant).cell_x, x0) << "should climb the food trail toward +x";
}
