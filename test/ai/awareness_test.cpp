// T-I9-AI: perception-fusion awareness state machine coverage — buildup, decay,
...
TEST(Awareness, StrongSignalEscalatesUnawareToEngaged) {
    ...
    for (int i = 0; i < 60; ++i) {
        ...
        if (a.state == AwarenessState::Suspicious) saw_suspicious = true;
        if (a.state == AwarenessState::Alert) saw_alert = true;
    }
...
TEST(Awareness, LosingSignalWhileAwareEntersSearchingThenForgets) {
    ...
    for (int i = 0; i < 60; ++i) a = UpdateAwareness(a, 1.0f, 5.0f, 5.0f, kDt, p);
    ...
    for (int i = 0; i < 200; ++i) a = UpdateAwareness(a, 0.0f, 0.0f, 0.0f, kDt, p);
    EXPECT_EQ(a.state, AwarenessState::Unaware);
...
TEST(Awareness, MeterIsClampedToUnitInterval) {
    AwarenessParams p;
    Awareness a;
    for (int i = 0; i < 1000; ++i) a = UpdateAwareness(a, 5.0f, 1.0f, 1.0f, kDt, p);
...
TEST(Awareness, FaintSignalReachesSuspiciousNotAlert) {
    ...
    for (int i = 0; i < 100; ++i) a = UpdateAwareness(a, 0.05f, 2.0f, 2.0f, kDt, p);
