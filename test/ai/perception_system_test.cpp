// T-I9-AI E1: PerceptionSystem coverage — vision/hearing fusion drives awareness
...
entt::entity MakePrey(entt::registry& r, float x, float z, std::uint32_t faction = 2,
                      float loud = 0.0f, float pitch = 0.5f) {
    const auto e = r.create();
    r.emplace<TransformComponent>(e).position = Luminumbra::Vec3(x, 0.0f, z);
...
TEST(PerceptionSystem, TargetOutsideVisionConeIsNotSeenButCanBeHeard) {
    ...
    for (int i = 0; i < 60; ++i) RunPerceptionSystemOnTick(r2, kDt);
...
TEST(PerceptionSystem, IsDeterministic) {
    auto run = []() {
        ...
        const auto pred = MakePredator(r, 0, 0, 1, 0);
        MakePrey(r, 12, 3);
        for (int i = 0; i < 25; ++i) RunPerceptionSystemOnTick(r, kDt);
